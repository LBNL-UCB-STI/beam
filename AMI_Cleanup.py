...
#TODO: Jenkins is set up to use Justin IAM - change to beambox
...
#Until then it should be setup as a cron job per below - to be executed once every 2 days
# sudo crontab -e
# */* * 2 * * /home/ubuntu/AMI_Cleanup.py >> /home/ubuntu/AMI_Cleanup.log 2>&1

utc=pytz.UTC

def getEC2For(region):
    ec2 = boto3.client('ec2', region_name=region)
    return ec2

def backup_and_retrieve_config():
...
def getTaggedAMIsAlongWithMostRecent(region):
    ec2 = getEC2For(region)
...
def restart_jenkins():
    process = subprocess.call(['sudo', 'service', 'jenkins', 'restart'])
...
def delete_older_amis_and_snapshots(allAMIsTagged, region):
    ...
    print('Deleting AMI IDs and correlated SnapshotIds for region '++':' + str(amisAndSnapshotsToDelete))
    ec2 = getEC2For(region)
...
def main():
    taggedAMIsAndMostRecent = getTaggedAMIsAlongWithMostRecent('us-east-2')
    config = backup_and_retrieve_config()
    ...
    if len(filter(lambda x: x[1] == False, taggedAMIsAndMostRecent[0])) >= 2: #Must keep >= 2 before can delete older ones
        ...
        for region in ['us-east-1','us-west-2']:
            secondaryRegionTaggedAMIsAndMostRecent = getTaggedAMIsAlongWithMostRecent(region)
