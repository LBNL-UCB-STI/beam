...
class UberSpeedGraph(
  ...
  private val (ways, nodes) = {
    val (w, n) = load(path.map(Paths.get(_)))
      .foldLeft((mutable.Map[String, Seq[WayMetric]](),
                 mutable.Map[UberWay, Seq[WayMetric]]())) {
        case ((accW, accN), s) =>
    ...
    (
      ...
      n.par
        .map {
          case (uw, v) =>
            (dictJ(uw.startJunctionId),
             dictJ(uw.endJunctionId),
             uw.segmentId,
             v)
        }
        .collect {
          case (Some(s), Some(e), sId, ws) => UberDirectedWay(s, e, sId, ws)
        }
    )
  }
  ...
  private val nodeGraph: Graph[Long, LkDiEdge] = Graph(
    nodes
      .map {
        case UberDirectedWay(s, e, sId, w) => (s ~+#> e)(WayMetrics(sId, w))
      }
      .seq
      .toSeq: _*
  )

  private def shorterPath(origNodeId: Long,
                          destNodeId: Long): Option[nodeGraph.Path] = {
    def shorter(p1: Option[nodeGraph.Path], p2: Option[nodeGraph.Path]) =
      if (p1.map(_.length).getOrElse(Integer.MAX_VALUE) == p2
            .map(_.length)
        ...
        if (mp1.size == mp2.size) {
          if (mp1
                .map(_.speedMphMean)
                .sum / mp1.size >= mp2.map(_.speedMphMean).sum / mp2.size) p1
          else p2
        } else if (mp1.size > mp2.size) p1
        else p2
      } else if (p1.map(_.length).getOrElse(Integer.MAX_VALUE) < p2
                   .map(_.length)
                   .getOrElse(Integer.MAX_VALUE)) p1
      else p2

    val path1 = nodeGraph
      .find(origNodeId)
      .flatMap(o =>
        nodeGraph.find(destNodeId).flatMap(d => o.shortestPathTo(d)))
    val path2 = nodeGraph
      .find(destNodeId)
      .flatMap(d =>
        nodeGraph.find(origNodeId).flatMap(o => d.shortestPathTo(o)))

    shorter(path1, path2)
  }

  override def speed(origNodeId: Long,
                     destNodeId: Long): Option[Seq[WayMetrics]] =
    for {
