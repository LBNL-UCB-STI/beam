...
object Wrapper {
  implicit class WrapperSyntax[F[_], A](fa: F[A]) {
    ...
    def flatMap[B](afb: A => F[B])(implicit F: Wrapper[F]): F[B] = F.flatMap(fa, afb)
    def zip[B >: A](fRight: F[A])(implicit F: Wrapper[F]): F[B] = F.zip(fa, fRight)
  }
...
trait ProgramInterprets {
  implicit object WrapperOption extends Wrapper[Option] {
    ...
    override def zip[A, B >: A](fLeft: Option[A], fRight: Option[A]): Option[B] = fLeft.orElse(fRight)
  }
}
