...
sealed trait WayFilter[T <: FilterDTO, E] {
  ...
  protected def parseHours(hours: Seq[UberHourSpeed]): WaySpeed = {
    val speedAvg = Try(hours.map(_.speedMean).sum / hours.size).getOrElse(0f)
    val devMax = Try(hours.map(_.maxDev).max).getOrElse(0f)
    val speedMean = Option(hours.map(_.speedMean).toArray).filter(_.nonEmpty).map(Median.findMedian).getOrElse(0f)
