...
class SfbayRouterSpec extends TestKit(ActorSystem("router-test")) with WordSpecLike
  ...
  "A router" must {
    ...
    "respond with a ride hailing route to a reasonable RoutingRequest" in {
      ...
      router ! RoutingRequest(RoutingRequestTripInfo(origin, destination, time, Vector(), Vector(StreetVehicle(Id.createVehicleId("rideHailingVehicle-person=17673-0"), new SpaceTime(new Coord(origin.getX, origin.getY), time.atTime), Modes.BeamMode.CAR, asDriver = false)), Id.createPersonId("81370-0")))
      val response = expectMsgType[RoutingResponse]
      System.out.println(response.itineraries.map(_.tripClassifier));
