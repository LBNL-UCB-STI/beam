...
//      }
    ...
    "correctly update previous iteration revenues and resetting current" in {
      ...
      rhspm.surgePriceBins.values.map(f =>
        f.map(
          _.currentIterationRevenue shouldBe (expectedResultCurrentIterationRevenue)))
    }
    ...
    "return correct surge level" in {
      ...
      val timeBinSize =
        beamConfig.beam.agentsim.agents.rideHail.surgePricing.timeBinSize
      val hourRandom = 1
      val hourInSeconds = hourRandom * timeBinSize

      val expectedValue =
        rhspm.surgePriceBins.get(randomTaz.tazId.toString).get.apply(hourRandom)
