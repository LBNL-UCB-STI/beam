...
object MatchingPerformanceTest extends BeamHelper {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val numRequestsPerVehicle = args(0).toInt
    val durInMin = args(1).toInt
    val nbVehicles = args(0).toInt
    ...
    println("Sum Of Delays: " + assignments.map(_.sumOfDelays).sum)
    println("END")
  }
