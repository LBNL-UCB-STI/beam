      ...
      "construct a flatfee pricing model" in {
        ...
        PricingModel("flatfee", inputCost.toString) match {
          case Some(PricingModel.FlatFee(cost, intervalInSeconds)) =>
            ...
            PricingModel.evaluateParkingTicket(PricingModel.FlatFee(cost, intervalInSeconds), duration) should equal (inputCost)
          case _ => fail()
        }
      }
      "construct a block pricing model" in {
        ...
        PricingModel("block", inputCost.toString) match {
          case Some(PricingModel.Block(cost, intervalInSeconds)) =>
            ...
            intervalInSeconds should equal (PricingModel.DefaultPricingInterval)
            PricingModel.evaluateParkingTicket(PricingModel.Block(cost, intervalInSeconds), duration) should equal (inputCost * 2)
          case _ => fail()
        }
      }
    }
        ...
        for {
          ...
          PricingModel("block", inputCost.toString, blockIntervalInSeconds.toString) match {
            case Some(PricingModel.Block(cost, intervalInSeconds)) =>
              ...
              val expectedTicketPrice: Double = inputCost.toDouble * (parkingDuration.toDouble / blockIntervalInSeconds.toDouble)
              PricingModel.evaluateParkingTicket(PricingModel.Block(cost, intervalInSeconds), parkingDuration) should equal (expectedTicketPrice)
            case _ => fail()
          }
        }
      }
