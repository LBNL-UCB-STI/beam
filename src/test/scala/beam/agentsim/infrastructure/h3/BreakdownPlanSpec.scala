...
class BreakdownPlanSpec extends WordSpec with Matchers {
  ...
  "BreakdownPlan" should {
    ...
    "NOT SPLIT whenever buckets has nothing to be split" in {
      val indexToStrings: Map[H3Index, Set[H3Point]] = Map(
        H3Index("81033ffffffffff") -> Set(H3Point(90.0, -99.61181940776801)),
        H3Index("81b57ffffffffff") -> Set(H3Point(-35.17155063934615, -163.2908517352389)),
        H3Index("81373ffffffffff") -> Set(H3Point(38.96802394893233, -148.88121342210718)),
      )
    ...
    "SPLIT considering distribution of points but also considering indexes cannot split are rounded up to 1" in {
      val indexA = H3Index("81033ffffffffff")
      val indexB = H3Index("81b57ffffffffff")
      val indexC = H3Index("81373ffffffffff")
      val indexAPoints = Set(
        H3Point(90.0, -99.61181940776801),
        H3Point(86.7016228907434, -169.10341629270596),
        H3Point(90.0, -15.161463921196875),
        H3Point(90.0, -40.57347268943054),
        H3Point(87.60904265202893, 153.57888248410174)
      )
      val content = H3Content(
        Map(
          indexA -> indexAPoints,
