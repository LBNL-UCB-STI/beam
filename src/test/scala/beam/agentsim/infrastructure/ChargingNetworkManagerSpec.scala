...
class ChargingNetworkManagerSpec
  ...
  "ChargingNetworkManager" should {
    ...
    "add a vehicle to charging queue with full fuel level but ends up with no fuel added" in {
      ...
      chargingNetworkManager ! ChargingPlugRequest(10, beamVilleCar, parkingStall, personId, self)
      ...
      chargingNetworkManager ! ChargingUnplugRequest(610, personId, beamVilleCar)
    ...
    "add a vehicle to charging queue with some fuel required and will charge" in {
      ...
      expectMsgType[EndingRefuelSession] shouldBe EndingRefuelSession(442, beamVilleCar.id)
      expectMsgType[CompletionNotice]
      expectNoMessage()
      beamVilleCar.primaryFuelLevelInJoules should be(2.16e8)

      chargingNetworkManager ! ChargingUnplugRequest(500, personId, beamVilleCar)
    ...
    "add a vehicle to charging queue with a lot fuel required and will charge in 2 cycles" in {
      ...
      expectMsgType[EndingRefuelSession] shouldBe EndingRefuelSession(334, beamVilleCar.id)
      expectMsgType[CompletionNotice]
      expectNoMessage()
      beamVilleCar.primaryFuelLevelInJoules should be(2.16e8)

      chargingNetworkManager ! ChargingUnplugRequest(700, personId, beamVilleCar)
    ...
    "add a vehicle to charging queue with a lot fuel required but unplug event happens before 1st cycle" in {
      ...
      chargingNetworkManager ! ChargingUnplugRequest(35, personId, beamVilleCar)
    ...
    "add a vehicle to charging queue with a lot fuel required but unplug event happens after 1st cycle" in {
      ...
      chargingNetworkManager ! ChargingUnplugRequest(315, personId, beamVilleCar)
    ...
    "add a vehicle to charging queue with a lot fuel required but unplug event happens after 2nd cycle" in {
      ...
      val beamVilleCar = getBeamVilleCar("2", parkingStall, 0.8)
      chargingNetworkManager ! ChargingPlugRequest(100, beamVilleCar, parkingStall, personId, self)
      expectMsgType[StartingRefuelSession]
      expectNoMessage()
      beamVilleCar.primaryFuelLevelInJoules should be(5.4e7)
      ...
      expectMsgType[EndingRefuelSession] shouldBe EndingRefuelSession(748, beamVilleCar.id)
      expectMsgType[CompletionNotice]
      expectNoMessage()
      beamVilleCar.primaryFuelLevelInJoules should be(2.16e8)

      chargingNetworkManager ! ChargingUnplugRequest(750, personId, beamVilleCar)
    ...
    "add two vehicles to charging queue and ends both charged" in {
      ...
      chargingNetworkManager ! ChargingPlugRequest(10, beamVilleCar2, parkingStall, personId, self)
      expectMsgType[StartingRefuelSession]
      expectNoMessage()
      beamVilleCar2.primaryFuelLevelInJoules should be(1.08e8)

      chargingNetworkManager ! ChargingPlugRequest(10, beamVilleCar3, parkingStall, personId, self)
      expectMsgType[WaitingToCharge] should be(WaitingToCharge(10, beamVilleCar3.id, parkingStall, 1))
      expectNoMessage()
      beamVilleCar3.primaryFuelLevelInJoules should be(1.08e8)
      ...
      expectMsgType[EndingRefuelSession] shouldBe EndingRefuelSession(442, beamVilleCar2.id)
      expectMsgType[CompletionNotice]
      expectNoMessage()
      ...
      chargingNetworkManager ! ChargingUnplugRequest(750, personId, beamVilleCar2)
      ...
      expectMsgType[EndingRefuelSession] shouldBe EndingRefuelSession(874, beamVilleCar3.id)
      expectMsgType[CompletionNotice]
      expectNoMessage()
      beamVilleCar2.primaryFuelLevelInJoules should be(2.16e8)
