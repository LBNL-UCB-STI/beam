...
class H3TAZSpec extends FlatSpec with Matchers with BeamHelper {
  ...
  private def testH3(config: Config): Unit = {
    ...
    val matsimScenario = ScenarioUtils.loadScenario(matsimConfig).asInstanceOf[MutableScenario]
    val networkCoordinator = DefaultNetworkCoordinator(beamConfig)
    networkCoordinator.loadNetwork()
    networkCoordinator.convertFrequenciesToTrips()
    matsimScenario.setNetwork(networkCoordinator.network)
    val tazMap = TAZTreeMap.fromShapeFile("test/input/sf-light/shape/sf-light-tazs.shp", "taz")
    val hexMap = new H3TAZ(networkCoordinator.network, tazMap, beamConfig)
    val popPerHex = H3TAZ.breakdownByPopulation(matsimScenario, hexMap, 10)
    assert(popPerHex.foldLeft(0)(_ + _._3.toInt) == matsimScenario.getPopulation.getPersons.size)
