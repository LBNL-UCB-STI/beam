...
class TransitInitializerSpec extends WordSpecLike with Matchers with MockitoSugar with IntegrationSpecCommon {
  "getVehicleType" should {
    ...
    "return SUV, based on agency[217] and route[1342] map" in {
      ...
      val actualType = transitInitializer.getVehicleType(routeInfo("217", "1342"), BeamMode.BUS).id
    ...
    "return RAIL-DEFAULT, based on agency[DEFAULT] " in {
      ...
      val actualType = transitInitializer.getVehicleType(routeInfo("DEFAULT", "dummy"), BeamMode.RAIL).id
    ...
    "return BUS-DEFAULT, as a default vehicle type" in {
      ...
      val actualType = transitInitializer.getVehicleType(routeInfo("dummy", "dummy"), BeamMode.BUS).id
    ...
    "not be BUS-AC, as vehicleTypes doesn't have it" in {
      ...
      val actualType = transitInitializer.getVehicleType(routeInfo("217", "1350"), BeamMode.BUS).id
