...
class TimeDependentRoutingSpec
    extends TestKit(
      ActorSystem("router-test", testConfig("test/input/beamville/beam.conf")))
    with WordSpecLike
    with Matchers
    with ImplicitSender
    with MockitoSugar
  ...
  "A time-dependent router" must {
    ...
    "take given link traversal times into account" in {
      ...
      router ! UpdateTravelTime(
        (_: Link, _: Double, _: Person, _: Vehicle) => 0) // Nice, we can teleport!
      router ! RoutingRequest(
        origin,
        destination,
        time,
      ...
      router ! UpdateTravelTime((_: Link, _: Double, _: Person, _: Vehicle) =>
        1000) // Every link takes 1000 sec to traverse.
      router ! RoutingRequest(
        origin,
        destination,
        time,
    ...
    "find an equilibrium between my estimation and my experience when I report my self-decided link travel times back to it" in {
      ...
      val travelTimeCalculator = new TravelTimeCalculator(
        networkCoordinator.network,
      ...
      for (_ <- 1 to 5) {
        RoutingModel
          .traverseStreetLeg(carOption.legs(0).beamLeg,
                             vehicleId,
                             longerTravelTimes)
