...
class StartWithCustomConfig(val config: Config)
    ...
    with BeamHelper {
  val conf = config.withValue("matsim.modules.controler.lastIteration",
                              ConfigValueFactory.fromAnyRef(0))
  val beamConfig = BeamConfig(conf)

  val (matsimConfig, _) = runBeamWithConfig(conf)

  val file: File = getEventsFilePath(
    matsimConfig,
    beamConfig.beam.outputs.events.fileOutputFormats)

  val eventsReader: ReadEvents = new ReadEventsBeam

  val listValueTagEventFile = eventsReader.getListTagsFrom(file.getPath,
                                                           tagToReturn = "mode",
                                                           eventType =
                                                             Some("ModeChoice"))
