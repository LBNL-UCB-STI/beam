...
public class RideHailWaitingAnalysis implements GraphAnalysis, IterationSummaryAnalysis {
    ...
    private int rideHailCount = 0;   //later used to calculate average wait linkStartTime experienced by customers
    ...
    private void processRideHailWaitingTimes(ModeChoiceEvent event, double waitingTime) {
        ...
        if (simMetricCollector.metricEnabled("ride-hail-waiting-linkStartTime-map")) {
            try {
                int linkId = Integer.parseInt(event.location);
                Coord coord = geo.coordOfR5Edge(transportNetwork.streetLayer, linkId);
                simMetricCollector.writeIterationMapPoint("ride-hail-waiting-linkStartTime-map", event.getTime(), waitingTime, coord.getY(), coord.getX(), false);
    ...
    private void writeWaitingTimeToStats(Map<Integer, List<Double>> hourToWaitings, List<Double> categories) {
        if (simMetricCollector.metricEnabled("ride-hail-waiting-linkStartTime")) {
            ...
            hourToCategories.forEach((hour, catToCnt) -> catToCnt.forEach((category, count) -> {
                ...
                simMetricCollector.writeIterationJava("ride-hail-waiting-linkStartTime", hour * 60 * 60, count, tags, true);
            }));
        }
    }
