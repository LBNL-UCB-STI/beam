...
public class RideHailWaitingTazAnalysis implements GraphAnalysis {
    ...
    public void processStats(Event event) {
        ...
        enters a vehicle , compute the difference between the times of occurrence for both the events as `waiting linkStartTime`.*/
        if (event instanceof ReserveRideHailEvent) {
            ...
            if (rideHailWaitingQueue.containsKey(personId.toString()) && personEntersVehicleEvent.getVehicleId().toString().contains("rideHailVehicle")) {
                //process and add the waiting linkStartTime to the total linkStartTime spent by all the passengers on waiting for a ride hail
    ...
    /**
     * Adds the given waiting linkStartTime & TAZ to the list of cumulative waiting times per bin.
     * @param event An occurred event
     * @param waitingTime linkStartTime spent by the passenger on waiting for the vehicle
    ...
    private void processRideHailWaitingTimesAndTaz(Event event, double waitingTime,TAZ taz) {
        ...
        //Add new / update the waiting linkStartTime details to the list of cumulative waiting times per bin
