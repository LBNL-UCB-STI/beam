    ...
    public void createGraphs(IterationEndsEvent event) {
            try {
                ...
                if (CONTROLLER_IO != null) {
                    ...
                    if (beamConfig.beam().calibration().mode().benchmarkFilename().trim().length() > 0) {
                        String outPath = CONTROLLER_IO.getOutputFilename(ModeChosenAnalysis.getModeChoiceFileBaseName() + ".csv");
                        Double modesAbsoluteError = new ModeChoiceObjectiveFunction(beamConfig.beam().calibration().mode().benchmarkFilename())
                                .evaluateFromRun(outPath, ErrorComparisonType.AbsoluteError());
                        log.info("modesAbsoluteError: " + modesAbsoluteError);

                        Double modesRMSPError = new ModeChoiceObjectiveFunction(beamConfig.beam().calibration().mode().benchmarkFilename())
