...
import beam.playground.physicalSimProtoType.AkkaJDEQSim.messages.EnterRoadMessage;
...
public class Vehicle extends SimUnit {
	...
	public Vehicle(ActorRef scheduler, Person ownerPerson, PlansConfigGroup.ActivityDurationInterpretation activityDurationInterpretation){
		super(scheduler);
	...
	public void initialize() {
		...
		ActorRef road = Road.getRoad(getCurrentLinkId());
	...
	protected void _scheduleEnterRoadMessage(double scheduleTime, ActorRef road) {
		sendMessage(new EnterRoadMessage(scheduler, this), road, scheduleTime);
	}

	public void scheduleEndRoadMessage(double scheduleTime, ActorRef road) {
		sendMessage(MessageFactory.getEndRoadMessage(road.scheduler, this), road, scheduleTime);
	}

	public void scheduleLeaveRoadMessage(double scheduleTime, ActorRef road) {
		sendMessage(MessageFactory.getLeaveRoadMessage(road.scheduler, this), road, scheduleTime);
	}

	public void scheduleEndLegMessage(double scheduleTime, ActorRef road) {
		sendMessage(MessageFactory.getEndLegMessage(road.scheduler, this), road, scheduleTime);
	}

	public void scheduleStartingLegMessage(double scheduleTime, ActorRef road) {
		sendMessage(MessageFactory.getStartingLegMessage(road.scheduler, this), road, scheduleTime);
	}

	public DeadlockPreventionMessage scheduleDeadlockPreventionMessage(double scheduleTime, ActorRef road) {
