...
@Singleton
class DumpDataAtEndImpl implements DumpDataAtEnd, ShutdownListener {
		...
		try {
			if (transitSchedule != null) {
				...
				//	log.info("re-projecting transit schedule from " + internalCRS + " back to " + inputCRS + " for export");
					...
					new TransitScheduleWriter(transformation, transitSchedule)
							.writeFile(controlerIO.getOutputFilename("output_transitSchedule.xml.gz"));
				}
			}
		} catch (Exception ee) {
		}
