...
package beam.controller.corelisteners;
...
public class EventsHandlingImpl implements EventsHandling, BeforeMobsimListener, AfterMobsimListener, IterationEndsListener, ShutdownListener {
	...
	public void notifyBeforeMobsim(BeforeMobsimEvent event) {
		...
		if (writingEventsAtAll && (regularWriteEvents || earlyIteration)) {
			if (EVGlobalData.data.EVENTS_FILE_OUTPUT_FORMATS.contains("xml")) {
				String xmlEventFileName = "events.xml";
				if (EVGlobalData.data.EVENTS_FILE_OUTPUT_FORMATS.contains("xml.gz")) {
					xmlEventFileName += ".gz";
				}
				this.eventWriters.add(new EVEventWriterXML(controlerIO.getIterationFilename(event.getIteration(), xmlEventFileName)));
			}

			if (EVGlobalData.data.EVENTS_FILE_OUTPUT_FORMATS.contains("csv")) {
				String csvEventFileName = "events.csv";
				if (EVGlobalData.data.EVENTS_FILE_OUTPUT_FORMATS.contains("csv.gz")) {
					csvEventFileName += ".gz";
				}
