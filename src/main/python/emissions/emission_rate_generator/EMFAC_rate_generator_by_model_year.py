...
analysis_year = [2018, 2025, 2030, 2040, 2050]
emission_rate_main = read_csv('Input/PL_MTC_2018_2025_2030_2040_2050_Annual_20240612153547.csv')
print(emission_rate_main.columns)
NH3_emission_rate = read_csv('Input/EMFAC_2018-2050_NH3_rate_modelyear.csv')
...
for yr in analysis_year: # loop through calendar year
    ...
    for vt in vehicle_types:
        ...
        for ft in fuel_types:
            ...
            for my in model_years:
                ...
                NH3_emission_rate_by_my = \
                    NH3_emission_rate_by_my.loc[NH3_emission_rate_by_my['Total VMT'] > 0] # if VMT is 0, the emission rate would be zero
                ...
                merger_attr = ['Vehicle Category', 'Fuel', 'Model Year', 'Region', 'Speed']
                NH3_emission_rate_imp = pd.merge(NH3_emission_rate_by_my, 
                                                 speed_bins_df,
                                                  on = merger_attr, how = 'right')
                NH3_emission_rate_imp.loc[:,'Calendar Year'].fillna(yr, inplace = True)
                NH3_emission_rate_imp.loc[:,'Model Year'].fillna(my, inplace = True)
    ...
    NH3_emission_rate_out = NH3_emission_rate_out.drop(columns = ['Region', 'Total VMT'])
    ...
    merger_var = ['calendar_year', 'vehicle_class', 'fuel', 'model_year', 'speed_time']
    ...
    emission_rate_out.to_csv('Output/imputed_MTC_emission_rate_agg_NH3_added_' + str(yr) + '.csv', index = False)
    # break