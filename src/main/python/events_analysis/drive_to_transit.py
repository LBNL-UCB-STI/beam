...
def find_num_drive_to_transit_agents(df):
    ...
    1.  During ModeChoice event, at linkStartTime t, person i chooses mode='transit'
    2.  During PathTraversal event, at linkStartTime t'>t, Person i completes
    ...
    for idx, row in df.iterrows():
        ...
        if row['type'] == 'ModeChoice' and row['mode'] == 'transit':
            ...
            dtt_dict[person_id] = int(row['linkStartTime'])
        ...
        # check)
            ...
            if pid_from_vid is not None:
                ...
                if pid_match in dtt_dict and (dtt_dict[pid_match] != 0 and int(row['linkStartTime']) > dtt_dict[pid_match]):
                    total += 1
