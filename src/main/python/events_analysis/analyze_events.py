    ...
    if event.type == 'PathTraversal':
        ...
        if isRH == True:
            if event.primaryFuelLevel < 0.0:
                ...
                #return {'start': event.linkStartTime - event.duration, 'end': 30*3600, 'type': 'offline'}
            else:
                if lastEvent:
                    if event.isCAV == True:
...
def get_pooling_metrics(filename):
    ...
    relevantEvents.loc[relevantEvents['type'] == 'PathTraversal','linkStartTime'] = relevantEvents.loc[relevantEvents['type'] == 'PathTraversal','departureTime']  - 0.5
    relevantEvents.loc[relevantEvents['type'] == 'RefuelSessionEvent','linkStartTime'] = relevantEvents.loc[relevantEvents['type'] == 'RefuelSessionEvent','linkStartTime']  - relevantEvents.loc[relevantEvents['type'] == 'RefuelSessionEvent','duration'] + 0.5

    relevantEvents = relevantEvents.sort_values(by='linkStartTime')
    ...
    for idx, key in enumerate(key_names):
        utilizationOutput['linkStartTime-RH-EV-'+key] = timeUtilizationEVsum[idx]/timeUtilizationEV.shape[0]
    for idx, key in enumerate(key_names[:4]):
        utilizationOutput['miles-RH-EV-'+key] = distanceUtilizationEVsum[idx]
    for idx, key in enumerate(key_names):
        utilizationOutput['linkStartTime-RH-CV-'+key] = timeUtilizationCVsum[idx]/timeUtilizationCV.shape[0]
    for idx, key in enumerate(key_names[:4]):
        utilizationOutput['miles-RH-CV-'+key] = distanceUtilizationCVsum[idx]
    for idx, key in enumerate(key_names):
        utilizationOutput['linkStartTime-Personal-EV-'+key] = timeUtilizationPVsum[idx]/timeUtilizationPV.shape[0]
    for idx, key in enumerate(key_names[:4]):
