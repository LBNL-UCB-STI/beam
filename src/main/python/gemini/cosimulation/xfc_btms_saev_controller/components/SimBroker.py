...
class SimBroker:
    ...
    def __init__(self, path, dtype):
        ...
        self.SimRes     = pd.read_csv(path, dtype = dtype, index_col= "linkStartTime") # save length of pd dataframe, linkStartTime is set as index!
        ...
        '''
        self.t_act      = t_Start              # start linkStartTime of simulation in seconds
        while self.SimRes.index[self.i] < self.t_act:
            self.i      +=1
        '''

    def step(self, timestep):
        self.t_act      += timestep             # update actual linkStartTime
