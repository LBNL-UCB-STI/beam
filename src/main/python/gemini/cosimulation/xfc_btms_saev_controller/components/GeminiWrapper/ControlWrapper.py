...
class ControlWrapper:
    ...
    def __init__(self, initMpc, t_start, timestep_intervall, result_directory, RideHailDepotId, ChBaMaxPower, ChBaParkingZoneId, ChBaNum, path_BeamPredictionFile, dtype_Predictions, t_max)  -> None:
        ...
        if isinstance(self.ChargingStation, components.ChaDepMpcBase):
            ...
            a = 50/30 * ((self.SimBroker.t_max - self.SimBroker.t_act)/3.6e3) / 24  # demand charge cost
        ...
        self.ResultWriter.saveChargingStationProperties([self.ChargingStation])
        # this function takes a list of
        # charging station objects as input argument

    def synchronizeVehiclesAtStation(self, vehicleIdsAtStation, t_act) -> None:
        # this is a routine to remove vehicles from the charging station list ChBaVehicles and Queue, which are not at
        # the charging station anymore (in the list of vehicleIds we got from BEAM)
        ...
        for vehicle in vehicleIdsAtStation:
            ...
            if vehicle not in vehicleInDepot:
                ...
                logging.warning("Vehicle with id " + str(vehicle) + " is not in the charging station. "
                                                                    "This should not happen. "
                                                                    "But this vehicle will be removed now.")
        ...
        for vehicleId in pop_out:
            ...
            for i in range(0, len(self.ChargingStation.ChBaVehicles)):
                if self.ChargingStation.ChBaVehicles[i].vehicleId == vehicleId:
                    ...
                    self.ChargingStation.ChBaVehicles[i] = False
                    # set the list at the index to false, so that the vehicle is removed
                    break
            ...
            self.ResultWriter.forcedReleaseEvent(t_act, removedVehicle, self.ChargingStation.ChargingStationId)
    ...
    def step (self, timestep, t_act, GridPowerUpper, GridPowerLower, BtmsEnergy):
        ...
        self.ChargingStation.updateFromPhySim(BtmsEnergy)
