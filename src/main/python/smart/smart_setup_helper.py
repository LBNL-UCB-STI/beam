import analyze_events_v2
import pandas as pd
import os
import ssl
import urllib.request
from pandas.io.json import json_normalize
import json

# Helpers


def download_events(__url, __local_file_path, __iteration):
    downloaded_file = '{}.events.csv.gz'.format(__local_file_path)
    if not os.path.exists(downloaded_file):
        if not os.path.exists(__local_file_path.rsplit("/", 1)[0]):
            os.makedirs(__local_file_path.rsplit("/", 1)[0])
        if (not os.environ.get('PYTHONHTTPSVERIFY', '') and
                getattr(ssl, '_create_unverified_context', None)):
            ssl._create_default_https_context = ssl._create_unverified_context
        url = "{0}/ITERS/it.{1}/{1}.events.csv.gz".format(__url, __iteration)
        print("downloading {} ...".format(url))
        urllib.request.urlretrieve(url, downloaded_file)
    return downloaded_file


def download_stats(__url, __local_file_path):
    downloaded_file = '{}.summaryStats.csv'.format(__local_file_path)
    if not os.path.exists(downloaded_file):
        if not os.path.exists(__local_file_path.rsplit("/", 1)[0]):
            os.makedirs(__local_file_path.rsplit("/", 1)[0])
        if (not os.environ.get('PYTHONHTTPSVERIFY', '') and
                getattr(ssl, '_create_unverified_context', None)):
            ssl._create_default_https_context = ssl._create_unverified_context
        url = "{}/summaryStats.csv".format(__url)
        print("downloading {} ...".format(url))
        urllib.request.urlretrieve(url, downloaded_file)
    return downloaded_file


def download_beamLog(__url, __local_file_path):
    downloaded_file = '{}.beamLog.out'.format(__local_file_path)
    if not os.path.exists(downloaded_file):
        if not os.path.exists(__local_file_path.rsplit("/", 1)[0]):
            os.makedirs(__local_file_path.rsplit("/", 1)[0])
        if (not os.environ.get('PYTHONHTTPSVERIFY', '') and
                getattr(ssl, '_create_unverified_context', None)):
            ssl._create_default_https_context = ssl._create_unverified_context
        url = "{}/beamLog.out".format(__url)
        print("downloading {} ...".format(url))
        urllib.request.urlretrieve(url, downloaded_file)
    return downloaded_file


def get_metrics_from_stats(__url, __local_file_path, __iteration):
    summary_stats_file = download_stats(__url, __local_file_path)
    summary_stats_df = pd.read_csv(summary_stats_file, sep=",", index_col=None, header=0)
    return summary_stats_df[summary_stats_df['Iteration'] == __iteration]


def get_metrics_from_events(__url, __local_file_path, __iteration):
    metrics_json = analyze_events_v2.get_all_metrics(download_events(__url, __local_file_path, __iteration),
                                                     __local_file_path)
    with open("{}.all-metrics.json".format(__local_file_path), 'w') as outfile:
        json.dump(metrics_json, outfile)
    return pd.DataFrame.from_dict(json_normalize(metrics_json))


def get_metrics_from_beamLog(__url, __local_file_path):
    with open(download_beamLog(__url, __local_file_path)) as beamLog:
        for line in beamLog:
            if 'Number of person' in line and 'Removed' in line:
                return pd.DataFrame([(int(line.split(':')[1].split('.')[0].strip()))], columns=['population'])
    return pd.DataFrame(columns=['population'])


def get_metrics(__setup):
    __index = ['Rank', 'Year', 'Scenario', 'Technology', 'Iteration']
    final_output_df = pd.DataFrame()
    for (rank, year, scenario, technology, scenario_tech, remote_folder) in __setup['scenarios']:
        for iteration in __setup['iterations']:
            local_file_path = "{}/{}-{}".format(__setup['local_folder'], scenario_tech, year)
            local_file_path_itr = "{}-{}".format(local_file_path, iteration)
            local_metrics_file = "{}.metrics.csv".format(local_file_path_itr)
            if not os.path.exists(local_metrics_file):
                url = "{}/{}/{}".format(__setup['base_url'], scenario_tech, remote_folder)

                summary_stats_df = get_metrics_from_stats(url, local_file_path, iteration)
                summary_stats_df['Scenario'] = scenario
                summary_stats_df['Technology'] = technology
                summary_stats_df['Year'] = year
                summary_stats_df['Rank'] = rank
                summary_stats_df.set_index(__index)

                pool_metrics_df = get_metrics_from_events(url, local_file_path_itr, iteration)
                pool_metrics_df['Scenario'] = scenario
                pool_metrics_df['Technology'] = technology
                pool_metrics_df['Iteration'] = iteration
                pool_metrics_df['Year'] = year
                pool_metrics_df['Rank'] = rank
                pool_metrics_df.set_index(__index)

                beamLog_df = get_metrics_from_beamLog(url, local_file_path)
                beamLog_df['Scenario'] = scenario
                beamLog_df['Technology'] = technology
                beamLog_df['Iteration'] = iteration
                beamLog_df['Year'] = year
                beamLog_df['Rank'] = rank
                beamLog_df.set_index(__index)

                merged_metrics_df = pd.merge(pool_metrics_df, summary_stats_df, on=__index, how='inner')
                merged_metrics_df = pd.merge(merged_metrics_df, beamLog_df, on=__index, how='inner')

                # writing
                merged_metrics_df.set_index(__index).to_csv(local_metrics_file)

                # concat
                final_output_df = pd.concat([final_output_df, merged_metrics_df])
            else:
                final_output_df = pd.concat([final_output_df,
                                             pd.read_csv(local_metrics_file, sep=",", index_col=None, header=0)])
            print("{} ok!".format(remote_folder))
    return final_output_df


def make_plots(__setup_config_dict):
    years = list(set(x[1] for x in __setup_config_dict['scenarios']))
    if not os.path.exists("{}/{}.metrics-final.csv".format(__setup_config_dict['local_folder'], years[0])):
        final_output_df = get_metrics(__setup_config_dict)
        for year in years:
            local_metrics_file = "{}/{}.metrics-final.csv".format(__setup_config_dict['local_folder'], year)
            final_output_df[final_output_df['Year'] == year].sort_values(by=['Rank']).to_csv(local_metrics_file)
    for year in years:
        local_metrics_file = "{}/{}.metrics-final.csv".format(__setup_config_dict['local_folder'], year)
        #os.system("python3 makeplots.py {} {}/{}".format(final_output, local_metrics_file, year))
