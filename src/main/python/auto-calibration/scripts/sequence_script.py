...
for k in range(len(rel_nudge_stages)):  # per stage start x=(number of parallel pass 7 or 4) and 1(for bookkeeping) procs
    ...
    for m in range(parallel_passes):
        ...
        if k > 5:
            change_conf(input_vector_now[m], filename, finaliteration_flowcapacity)
        else:
            change_conf(input_vector_now[m], filename, finaliteration)
    ...
    if k > 5:
        bookkeep(k + 1, finaliteration_flowcapacity)
    else:
        bookkeep(k + 1, finaliteration)
