...
def change_conf(input_vector, filename, iteration, set_fcf = True):
...
def bookkeep(which_stage, iteration_num):
    ...
    for j in range(len(output_folders)):
        ...
        car_speed_df = pd.read_csv(car_speed)
        car_speed_avg = 0
        if len((car_speed_df[car_speed_df["iteration"] == iteration_num]).index) != 0:
            car_speed_avg = car_speed_df[(car_speed_df["iteration"] == iteration_num) | (car_speed_df["carType"] == 'Personal')]['avg'].values[0]
        car_speed_df = pd.read_csv(free_flow_car_speed)
        free_flow_car_speed_avg = 0
        if len((car_speed_df[car_speed_df["iteration"] == iteration_num]).index) != 0:
            free_flow_car_speed_avg = car_speed_df[(car_speed_df["iteration"] == iteration_num) | (car_speed_df["carType"] == 'Personal')]['avg'].values[0]
