...
import time, psutil, multiprocessing, os
from worker import fire_BEAM, bookkeep, recipe, start_beam, process_fcf_results
from config import *
...
for k in range(len(rel_nudge_stages)): # per stage start x=(number of parallel pass 7 or 4) and 1(for bookkeeping) procs
    ...
    parallel_passes = list(range(init_runs-1)) if k ==0 else list(range(parallel_run))
