...
def _save_graph_xml(  # noqa: C901
    ...
    for table in (gdf_nodes, gdf_edges):
        ...
        table["version"] = "1"
        table["changeset"] = "1"
        table["timestamp"] = ox.utils.ts(template="{:%Y-%m-%dT%H:%M:%SZ}")
    ...
    root = ET.Element(
        "osm", attrib={"version": str(api_version), "generator": f"OSMnx {ox.__version__}"}
    )
    ...
    ox.utils.log(f"Saved graph as .osm file at {filepath!r}")
...
def _get_unique_nodes_ordered_from_way(df_way_edges):
    ...
    H = ox.truncate.largest_component(G, strongly=False)
