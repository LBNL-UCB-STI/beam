...
def generateGraph(quad_output_file, repositioning_file):
    ...
    quad_data = {}
    quad_csv = open(quad_output_file)
    ...
    coord_output = {}
    movement_csv = open(repositioning_file)
    ...
    for time,points in coord_output.items():
        x1 = []
        y1 = []
        x2 = []
        y2 = []
        ...
        plt.plot(x3,y3, 'bo')
        plt.title("Time "+time)
        plt.savefig(inter('file_{time}.png'))
        plt.cla()


    for time,points in quad_data.items():
        x1 = []
        y1 = []
        for point in points:
            x1.append(point[0])
