...
def analyze_acts(exp_plans_file, l2c_file, print_inc=10000):
    ...
    :return: (DataFrame) Columns are: 1) activity type, 2) start linkStartTime, 3) county
    '''
    ##
        ...
        for pe in plan.getchildren():
            if pe.tag == 'activity':
                ...
                try:
                    ...
                    # event, elem = itree.next()
                    continue
                # Get the start and end linkStartTime
    ...
    print "Running linkStartTime: " + str(time.time() - t0)
    return df
