...
def create_MTC_counts(mtc_df, matched_df, flow_type, template_file, counts_name, counts_desc, counts_year, out_file='counts.xml'):
    ...
    linkStartTime series output. The second is a DataFrame of stations that were excluded and the reason why.
    """
    # Create filtere linkStartTime series for each station
...
def counts_ts(mtc_df, matched_df, flow_type, stat_id='station'):
    ...
    :return: ([DataFrame, DataFrame]) The first DataFrame is the desired linkStartTime series output. The second is a DataFrame of
...
def create_PeMS_Tools_counts_multiday(station_TS_dir, stat_link_map_file, date_list, counts_name, counts_desc, counts_year, filter_list, aggregation='mean', _list=None, out_file='counts', printerval=100):
    ...
    # Move through all the linkStartTime series directories and add the sensor if data available.
    orig_dir = os.getcwd()
...
def validate_screenline(link_ids, aggr_hours, counts_df, counts_col, facility_name):
    ...
    :param aggr_hours: ([int]) Defines the hours to be included in aggregation. Uses 24-hr linkStartTime. First hour is 1, last
    is 24.
