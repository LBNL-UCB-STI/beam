...
# Plots hierarchical clustering of count station deviations from some measure.
...
if __name__ == '__main__':
    ...
    # Z_days = sch.dendrogram(Y_days, distance_sort='ascending')
    # Z = sch.dendrogram(Y, p=5, truncate_mode='level', distance_sort='ascending')
    ax1.set_xticks([])
    ax1.set_yticks([])

    #TODO plot the days dendgrogram on the left side. Resort the days to match
    ...
    axmatrix.set_xticks([])
    axmatrix.set_yticks(range(df.shape[0]))  # Use dates
    axmatrix.set_yticklabels(df.iloc[:, 0])
