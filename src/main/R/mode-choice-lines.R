...
target <- data.frame(mode=rep(c('car','walk','transit','ride_hailing'),length(u(toplot$scenario))),perc=rep(c(79,4,13,5),length(u(toplot$scenario))),scenario=rep(u(toplot$scenario),each=length(u(toplot$mode))))
