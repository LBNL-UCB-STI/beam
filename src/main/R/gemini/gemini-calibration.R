...
chargingBehaviorFunc <- function(DT) {
  rseSum <- DT[,.(fuelMWH=sum(fuel)/3600000000),by=.(parkingType,chargingPointType)]
  rseSum$fuelShare <- round(rseSum$fuelMWH / sum(rseSum$fuelMWH), 3)
  print("************************")
  print(rseSum)
  print(paste("Total All (GWH): ", sum(rseSum$fuelMWH)/1000, sep=""))
  totRidehailGWH <- sum(DT[grepl("rideHail", vehicle),]$fuel)/(3600000*1000000)
  print(paste("Total Ridehail (GWH): ", totRidehailGWH, sep=""))
}

chargingBehaviorFunc2 <- function(DT) {
  rseSum <- DT[,.(fuel=sum(fuel)),by=.(parkingType,chargingPointType)]
  rseSum$chargingPointType2 <- "NA"
  rseSum[grepl("fc", chargingPointType)]$chargingPointType2 <- "FC"
  rseSum[grepl("depot", chargingPointType)]$chargingPointType2 <- "Depot"
  rseSum[grepl("publiclevel", chargingPointType)]$chargingPointType2 <- "Public"
  rseSum[grepl("homelevel", chargingPointType)]$chargingPointType2 <- "Home"
  rseSum[grepl("worklevel", chargingPointType)]$chargingPointType2 <- "Work"
  rseSum <- rseSum[,.(fuelMWH=sum(fuel)/3600000000),by=.(chargingPointType2)]
  rseSum$fuelShare <- round(rseSum$fuelMWH / sum(rseSum$fuelMWH), 3)
  print("************************")
  print(rseSum)
  print("************************")
  print(paste("Total All (GWH): ", round(sum(rseSum$fuelMWH)/1000, 3), sep=""))
}
...
events1 <- readCsv(pp(workDir, "/2022-07-05/events/filtered.0.events.a.csv.gz"))
...
events5 <- readCsv(pp(workDir, "/2022-07-05/events/filtered.0.events.7Advanced.csv.gz"))
events6 <- readCsv(pp(workDir, "/2022-07-05/events/filtered.0.events.8MaxEV.csv.gz"))
...
nrow(ref1[startsWith(person,"virtual")])
nrow(ref2[startsWith(person,"virtual")])


chargingBehaviorFunc(ref1)
chargingBehaviorFunc(ref2)
chargingBehaviorFunc(ref3)
chargingBehaviorFunc(ref4)

chargingBehaviorFunc2(ref1)
...
ref1[!startsWith(person, "virtual")]
ref1[person == "6262864"]
