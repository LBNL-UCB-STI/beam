...
extractChargingSessions <- function(events.file) {
  ...
  ev1 <- events[type %in% c("RefuelSessionEvent")][
    ,c("vehicle", "time", "type", "parkingTaz", "chargingType", "parkingType", 
       "locationY", "locationX", "duration", "vehicleType", "person", "fuel", 
       "parkingZoneId", "activityLocationX", "activityLocationY")][
  ...
  ev[,plug.xfc:=(kw>=250)]
  sessions <- ev[chargingType!='None' & time/3600>=4
                 ,.(start.time,depot,plug.xfc,taz=parkingTaz,kw,
                    x=locationX,y=locationY,duration=duration/60,chargingType,
