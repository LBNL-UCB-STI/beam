...
:main
    ...
    for /f "tokens=1,2 delims=, " %%a in (%host_csv%) do (
        ...
        REM ssh -i "%%b" ubuntu@%%a grep %search_word% %search_path%
        scp -i "%%b" -r copyAllSigOptSuggestionsToS3.sh ubuntu@%%a:~/copyAllSigOptSuggestionsToS3.sh
        ssh -i "%%b" ubuntu@%%a chmod +x copyAllSigOptSuggestionsToS3.sh
        ssh -i "%%b" ubuntu@%%a ./copyAllSigOptSuggestionsToS3.sh
    )
