...
object BeamLoggingReceive {
  ...
  def withLabel(label: String, logLevel: LogLevel)(r: Receive)(implicit context: ActorContext): Receive = r match {
    ...
    case _                 => if (context.system.settings.AddLoggingReceive) new BeamLoggingReceive(None, r, Option(label), logLevel) else r
  }
...
class BeamLoggingReceive(source: Option[AnyRef], r: Receive, label: Option[String], logLevel: LogLevel)
                        (implicit context: ActorContext) extends LoggingReceive(source, r, label, logLevel) {
  override def isDefinedAt(o: Any): Boolean = {
    ...
    if (context.system.eventStream.logLevel >= logLevel) {
      ...
      val message = "###BEAM### received " + (if (handled) "handled" else "unhandled") + " message " + o + " from " + context
        .sender() +
        (label match {
