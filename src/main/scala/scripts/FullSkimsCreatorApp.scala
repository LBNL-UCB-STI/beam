...
/**
  ...
  *
  * Run with gradle:
  * ./gradlew execute -PmainClass=scripts.FullSkimsCreatorApp -PappArgs=["'--configPath', 'test/input/beamville/beam-with-fullActivitySimBackgroundSkims.conf', '--output', 'output.csv', '--input', 'input.csv', '--ODSkimsPath', 'ODSkimsBeamville.csv',  '--linkstatsPath', '0.linkstats.csv'"]
  */
object FullSkimsCreatorApp extends App with BeamHelper {
  ...
  def createOdSkimmer(beamServices: BeamServices, rows: Vector[ODRow]): ODSkimmer =
    new ODSkimmer(beamServices.matsimServices, beamServices.beamScenario, beamServices.beamConfig) {
      lazy val hours = BackgroundFullSkimsCreator.getPeakHoursFromConfig(beamServices)
