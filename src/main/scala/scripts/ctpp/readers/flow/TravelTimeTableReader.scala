...
import scripts.ctpp.models.ResidenceToWorkplaceFlowGeography.`PUMA5 To POWPUMA`
import scripts.ctpp.models.{FlowGeoParser, OD, ResidenceToWorkplaceFlowGeography}
import scripts.ctpp.readers.BaseTableReader
import scripts.ctpp.readers.BaseTableReader.Table.Flow.TravelTime
...
) extends BaseTableReader(dbInfo, TravelTime, Some(residenceToWorkplaceFlowGeography.level)) {
  /*
...
object TravelTimeTableReader {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val rdr = new TravelTimeTableReader(databaseInfo, `PUMA5 To POWPUMA`)
