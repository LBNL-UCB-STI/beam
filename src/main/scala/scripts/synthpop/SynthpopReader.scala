...
class SynthpopReader(val houseHoldsWithPeople: Seq[(File, File)]) {
  ...
  def read(): Map[Household, Seq[Person]] = {
    val (_, allHouseholdsWithPeople) = houseHoldsWithPeople.foldLeft((0, Map[Household, Seq[Person]]())) {
      case ((totalNumberOfPeople, resultMap), (hhFile, personFile)) =>
        ...
        val households =
          new HouseholdReader(hhFile.getAbsolutePath).read().groupBy(x => x.id).map { case (hhId, xs) =>
            hhId -> xs.head
          }
...
object SynthpopReader extends StrictLogging {

  def apply(folderToScan: String): SynthpopReader = {
    ...
    new SynthpopReader(hhWithPeople)
  }
}
