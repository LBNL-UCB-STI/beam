...
import beam.utils.data.synthpop._
import scripts.synthpop.generators.{
...
class SimpleScenarioGenerator(
  ...
  private val householdWithPersons: Map[Models.Household, Seq[Models.Person]] = {
    ...
    val temp: Seq[(Models.Household, Seq[Models.Person])] = SynthpopReader.apply(pathToSythpopDataFolder).read().toSeq
    // Adjust population
...
object SimpleScenarioGenerator extends StrictLogging {
  ...
  def main(args: Array[String]): Unit = {
    ...
    ./gradlew :execute -PmaxRAM=20 -PmainClass=scripts.SimpleScenarioGenerator -PappArgs=["
