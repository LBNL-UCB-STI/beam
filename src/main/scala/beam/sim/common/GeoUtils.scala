...
trait GeoUtils extends HasServices {
  lazy val utm2Wgs: GeotoolsTransformation = new GeotoolsTransformation(
    beamServices.beamConfig.beam.spatial.localCRS,
    "EPSG:4326")
  lazy val wgs2Utm: GeotoolsTransformation = new GeotoolsTransformation(
    "EPSG:4326",
    beamServices.beamConfig.beam.spatial.localCRS)

  def wgs2Utm(spacetime: SpaceTime): SpaceTime =
    SpaceTime(wgs2Utm(spacetime.loc), spacetime.time)
  ...
  def utm2Wgs(spacetime: SpaceTime): SpaceTime =
    SpaceTime(utm2Wgs(spacetime.loc), spacetime.time)
  ...
  def distLatLon2Meters(x1: Double,
                        y1: Double,
                        x2: Double,
                        y2: Double): Double =
    GeoUtils.distLatLon2Meters(x1, y1, x2, y2)

  def getNearestR5Edge(streetLayer: StreetLayer,
                       coord: Coord,
                       maxRadius: Double = 1E5): Int = {
  ...
  def snapToR5Edge(streetLayer: StreetLayer,
                   coord: Coord,
    ...
    if (theSplit == null) {
      ...
      new Coord(theSplit.fixedLon.toDouble / 1.0E7,
                theSplit.fixedLat.toDouble / 1.0E7)
    }
  }
  def getR5Split(streetLayer: StreetLayer,
                 coord: Coord,
                 maxRadius: Double = 1E5,
    ...
    while (theSplit == null && radius <= maxRadius) {
      theSplit =
        streetLayer.findSplit(coord.getY, coord.getX, radius, streetMode)
      radius = radius * 10
    }
    if (theSplit == null) {
      theSplit =
        streetLayer.findSplit(coord.getY, coord.getX, maxRadius, streetMode)
    }
    theSplit
  }
}
...
object GeoUtils {
  ...
  def distLatLon2Meters(x1: Double,
    ...
    val a = Math.sin(distX / 2) * Math.sin(distX / 2) + Math.cos(Math.toRadians(
      x1)) * Math.cos(Math.toRadians(x2)) * Math.sin(distY / 2) * Math.sin(
      distY / 2)
...
class GeoUtilsImpl @Inject()(override val beamServices: BeamServices)
    extends GeoUtils {}
