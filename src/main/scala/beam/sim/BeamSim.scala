...
class BeamSim @Inject()(
  ...
  override def notifyIterationEnds(event: IterationEndsEvent): Unit = {
    ...
    val matsimGeneratedGraphNameRegexes = List(s"legHistogram(.*)","experienced(.*)")
    FileUtils.getFile(new File(event.getServices.getControlerIO.getIterationPath(event.getIteration)))
      .listFiles()
