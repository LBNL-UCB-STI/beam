...
class BeamMobsim @Inject() (
  ...
  private def fillInSecondaryActivities(households: Households, fillInModes: Boolean = false): Unit = {
    ...
    def writeDebugCSV(): File = {
      ...
      val content =
        header ++ beamServices.skims.od_skimmer.skimsDebugCalculation.map(_.mkString("", FieldSeparator, LineSeparator))
      FileUtils.writeToFile(outputFile, content)
      val f = new File(outputFile)
      logger.info(
        s"Skims usage debugging written ${beamServices.skims.od_skimmer.skimsDebugCalculation.length} rows out."
      )
      f
    }
...
class BeamMobsimIteration(
  ...
  def buildActivityQuadTreeBounds(population: MATSimPopulation): QuadTreeBounds = {
    ...
    val activities =
      persons.flatMap(p => p.getSelectedPlan.getPlanElements.asScala.view).collect { case activity: Activity =>
        activity
      }
