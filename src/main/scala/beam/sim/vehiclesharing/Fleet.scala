...
trait FleetType {
  def props(beamServices: BeamServices, beamScenario: BeamScenario, parkingManager: ActorRef): Props
}

case class FixedNonReservingFleet(config: SharedFleets$Elm.FixedNonReserving) extends FleetType {
  override def props(beamServices: BeamServices, beamScenario: BeamScenario, parkingManager: ActorRef): Props = {
    ...
    val vehicleType = beamScenario.vehicleTypes.getOrElse(
