...
trait ActorInject {
  ...
  protected def injectActor[A <: Actor](
  ...
  protected def injectActor[A <: Actor](
    name: String
  )(implicit factory: ActorRefFactory, tag: ClassTag[A]): ActorRef =
  ...
  protected def injectTopActor[A <: Actor](name: String)(implicit tag: ClassTag[A]): ActorRef =
    injector
      .getInstance(classOf[ActorSystem])
      .actorOf(Props(classOf[ActorProducer[A]], injector, tag.runtimeClass), name)
