          ...
          object AllocationManager {
            ...
            object AlonsoMora {
              ...
              def apply(
                ...
                BeamConfig.Beam.Agentsim.Agents.RideHail.AllocationManager.AlonsoMora(
                  maxTravelTimeDelayAsFraction =
                    if (c.hasPathOrNull("maxTravelTimeDelayAsFraction")) c.getDouble("maxTravelTimeDelayAsFraction")
                    else 0.2,
                  maxWaitTimeInSec = if (c.hasPathOrNull("maxWaitTimeInSec")) c.getInt("maxWaitTimeInSec") else 360,
                  ratioOfSolutionSpaceToAvailability =
                    if (c.hasPathOrNull("ratioOfSolutionSpaceToAvailability"))
                      c.getDouble("ratioOfSolutionSpaceToAvailability")
                    else 1.0,
                  ratioOfSolutionSpaceToRequest =
                    if (c.hasPathOrNull("ratioOfSolutionSpaceToRequest")) c.getDouble("ratioOfSolutionSpaceToRequest")
                    else 1.0
                )
              }
            }
        ...
        def apply(c: com.typesafe.config.Config): BeamConfig.Beam.Agentsim.Taz = {
          BeamConfig.Beam.Agentsim.Taz(
            ...
            probabilityOfResidentialCharging =
              if (c.hasPathOrNull("probabilityOfResidentialCharging")) c.getDouble("probabilityOfResidentialCharging")
              else 1.0
          )
        }
      }
