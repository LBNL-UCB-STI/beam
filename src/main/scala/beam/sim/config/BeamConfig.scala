...
object BeamConfig {
  ...
  object Beam {
    ...
    object Router {
          ...
          BeamConfig.Beam.Router.Skim$Elm(
            ...
            count_skimmer =
              if (c.hasPathOrNull("count-skimmer"))
                scala.Some(BeamConfig.Beam.Router.Skim$Elm.CountSkimmer(c.getConfig("count-skimmer")))
              else None,
            keepKLatestSkims = if (c.hasPathOrNull("keepKLatestSkims")) c.getInt("keepKLatestSkims") else 1,
            ...
            origin_destination_skimmer =
              if (c.hasPathOrNull("origin-destination-skimmer"))
                scala.Some(
                  BeamConfig.Beam.Router.Skim$Elm.OriginDestinationSkimmer(c.getConfig("origin-destination-skimmer"))
                )
              else None,
      ...
      private def $_LBeamConfig_Beam_Router_Skim$Elm(
        cl: com.typesafe.config.ConfigList
      ): scala.List[BeamConfig.Beam.Router.Skim$Elm] = {
        import scala.collection.JavaConverters._
        cl.asScala
          .map(cv => BeamConfig.Beam.Router.Skim$Elm(cv.asInstanceOf[com.typesafe.config.ConfigObject].toConfig))
          .toList
      }
    }
