// generated by tscfg 0.9.4 on Sun Nov 17 18:01:51 PST 2019
...
object BeamConfig {
  case class Beam(
    abstractSkimmer : BeamConfig.Beam.AbstractSkimmer,
  ...
  object Beam {
    ...
    object AbstractSkimmer {
      ...
      def apply(c: com.typesafe.config.Config): BeamConfig.Beam.AbstractSkimmer = {
        BeamConfig.Beam.AbstractSkimmer(
          aggregateFunction            = if(c.hasPathOrNull("aggregateFunction")) c.getString("aggregateFunction") else "LATEST_SKIM",
          keepKLatestSkims             = if(c.hasPathOrNull("keepKLatestSkims")) c.getInt("keepKLatestSkims") else 1,
          odSkimmer                    = BeamConfig.Beam.AbstractSkimmer.OdSkimmer(if(c.hasPathOrNull("odSkimmer")) c.getConfig("odSkimmer") else com.typesafe.config.ConfigFactory.parseString("odSkimmer{}")),
    ...
    object Beamskimmer {
      def apply(c: com.typesafe.config.Config): BeamConfig.Beam.Beamskimmer = {
        BeamConfig.Beam.Beamskimmer(
          writeAllModeSkimsForPeakNonPeakPeriodsInterval = if(c.hasPathOrNull("writeAllModeSkimsForPeakNonPeakPeriodsInterval")) c.getInt("writeAllModeSkimsForPeakNonPeakPeriodsInterval") else 0,
          writeFullSkimsInterval                         = if(c.hasPathOrNull("writeFullSkimsInterval")) c.getInt("writeFullSkimsInterval") else 0,
          writeObservedSkimsInterval                     = if(c.hasPathOrNull("writeObservedSkimsInterval")) c.getInt("writeObservedSkimsInterval") else 0,
          writeObservedSkimsPlusInterval                 = if(c.hasPathOrNull("writeObservedSkimsPlusInterval")) c.getInt("writeObservedSkimsPlusInterval") else 0
        )
      }
    }
    ...
    object Router {
      case class Skim(
        aggregateFunction            : java.lang.String,
        keepKLatestSkims             : scala.Int,
        skimmers                     : scala.List[BeamConfig.Beam.Router.Skim.Skimmers$Elm],
        writeAggregatedSkimsInterval : scala.Int,
        writeSkimsInterval           : scala.Int
      )
      object Skim {
        case class Skimmers$Elm(
          count_skimmer              : scala.Option[BeamConfig.Beam.Router.Skim.Skimmers$Elm.CountSkimmer],
          name                       : java.lang.String,
          origin_destination_skimmer : scala.Option[BeamConfig.Beam.Router.Skim.Skimmers$Elm.OriginDestinationSkimmer],
          skimType                   : java.lang.String
        )
        object Skimmers$Elm {
          case class CountSkimmer(
            skimType : java.lang.String
          )
          object CountSkimmer {
            def apply(c: com.typesafe.config.Config): BeamConfig.Beam.Router.Skim.Skimmers$Elm.CountSkimmer = {
              BeamConfig.Beam.Router.Skim.Skimmers$Elm.CountSkimmer(
                skimType = if(c.hasPathOrNull("skimType")) c.getString("skimType") else "count-skim"
              )
            }
          }
          ...
          object OriginDestinationSkimmer {
            def apply(c: com.typesafe.config.Config): BeamConfig.Beam.Router.Skim.Skimmers$Elm.OriginDestinationSkimmer = {
              BeamConfig.Beam.Router.Skim.Skimmers$Elm.OriginDestinationSkimmer(
                skimType                                       = if(c.hasPathOrNull("skimType")) c.getString("skimType") else "od-skim",
                writeAllModeSkimsForPeakNonPeakPeriodsInterval = if(c.hasPathOrNull("writeAllModeSkimsForPeakNonPeakPeriodsInterval")) c.getInt("writeAllModeSkimsForPeakNonPeakPeriodsInterval") else 0,
                writeFullSkimsInterval                         = if(c.hasPathOrNull("writeFullSkimsInterval")) c.getInt("writeFullSkimsInterval") else 0
              )
            }
          ...
          def apply(c: com.typesafe.config.Config): BeamConfig.Beam.Router.Skim.Skimmers$Elm = {
            BeamConfig.Beam.Router.Skim.Skimmers$Elm(
              count_skimmer              = if(c.hasPathOrNull("count-skimmer")) scala.Some(BeamConfig.Beam.Router.Skim.Skimmers$Elm.CountSkimmer(c.getConfig("count-skimmer"))) else None,
              name                       = if(c.hasPathOrNull("name")) c.getString("name") else "origin-destination-skimmer",
              origin_destination_skimmer = if(c.hasPathOrNull("origin-destination-skimmer")) scala.Some(BeamConfig.Beam.Router.Skim.Skimmers$Elm.OriginDestinationSkimmer(c.getConfig("origin-destination-skimmer"))) else None,
              skimType                   = if(c.hasPathOrNull("skimType")) c.getString("skimType") else "od-skim"
            )
          }
        }
        ...
        def apply(c: com.typesafe.config.Config): BeamConfig.Beam.Router.Skim = {
          BeamConfig.Beam.Router.Skim(
            aggregateFunction            = if(c.hasPathOrNull("aggregateFunction")) c.getString("aggregateFunction") else "LATEST_SKIM",
            keepKLatestSkims             = if(c.hasPathOrNull("keepKLatestSkims")) c.getInt("keepKLatestSkims") else 1,
            skimmers                     = $_LBeamConfig_Beam_Router_Skim_Skimmers$Elm(c.getList("skimmers")),
            writeAggregatedSkimsInterval = if(c.hasPathOrNull("writeAggregatedSkimsInterval")) c.getInt("writeAggregatedSkimsInterval") else 0,
            writeSkimsInterval           = if(c.hasPathOrNull("writeSkimsInterval")) c.getInt("writeSkimsInterval") else 0
          )
        }
        private def $_LBeamConfig_Beam_Router_Skim_Skimmers$Elm(cl:com.typesafe.config.ConfigList): scala.List[BeamConfig.Beam.Router.Skim.Skimmers$Elm] = {
          import scala.collection.JavaConverters._
          cl.asScala.map(cv => BeamConfig.Beam.Router.Skim.Skimmers$Elm(cv.asInstanceOf[com.typesafe.config.ConfigObject].toConfig)).toList
        }
      }
    ...
    def apply(c: com.typesafe.config.Config): BeamConfig.Beam = {
      BeamConfig.Beam(
        abstractSkimmer = BeamConfig.Beam.AbstractSkimmer(if(c.hasPathOrNull("abstractSkimmer")) c.getConfig("abstractSkimmer") else com.typesafe.config.ConfigFactory.parseString("abstractSkimmer{}")),
        ...
        beamskimmer     = BeamConfig.Beam.Beamskimmer(if(c.hasPathOrNull("beamskimmer")) c.getConfig("beamskimmer") else com.typesafe.config.ConfigFactory.parseString("beamskimmer{}")),
        ...
        h3              = BeamConfig.Beam.H3(if(c.hasPathOrNull("h3")) c.getConfig("h3") else com.typesafe.config.ConfigFactory.parseString("h3{}")),
