          ...
          object Enroute {
            def apply(c: com.typesafe.config.Config): BeamConfig.Beam.Agentsim.Agents.Vehicles.Enroute = {
              BeamConfig.Beam.Agentsim.Agents.Vehicles.Enroute(
                noRefuelThresholdInPercent       = if(c.hasPathOrNull("noRefuelThresholdInPercent")) c.getDouble("noRefuelThresholdInPercent") else 0.8,
                refuelRequiredThresholdInPercent = if(c.hasPathOrNull("refuelRequiredThresholdInPercent")) c.getDouble("refuelRequiredThresholdInPercent") else 0.2
              )
            }
          }
