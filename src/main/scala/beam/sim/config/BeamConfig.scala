// generated by tscfg 0.9.4 on Fri Nov 22 18:28:57 PST 2019
...
object BeamConfig {
  ...
  object Beam {
    ...
    object AbstractSkimmer {
      ...
      def apply(c: com.typesafe.config.Config): BeamConfig.Beam.AbstractSkimmer = {
        BeamConfig.Beam.AbstractSkimmer(
          aggregateFunction =
            if (c.hasPathOrNull("aggregateFunction")) c.getString("aggregateFunction") else "LATEST_SKIM",
          keepKLatestSkims = if (c.hasPathOrNull("keepKLatestSkims")) c.getInt("keepKLatestSkims") else 1,
          odSkimmer = BeamConfig.Beam.AbstractSkimmer.OdSkimmer(
            if (c.hasPathOrNull("odSkimmer")) c.getConfig("odSkimmer")
            else com.typesafe.config.ConfigFactory.parseString("odSkimmer{}")
          ),
    ...
    object Router {
      ...
      object Skim {
        case class Skimmers$Elm(
          ...
          od_skimmer: scala.Option[BeamConfig.Beam.Router.Skim.Skimmers$Elm.OdSkimmer]
        )

        object Skimmers$Elm {
          case class CountSkimmer(
            name: java.lang.String,
            skimFileBaseName: java.lang.String,
          ...
          object CountSkimmer {
            ...
            def apply(c: com.typesafe.config.Config): BeamConfig.Beam.Router.Skim.Skimmers$Elm.CountSkimmer = {
              BeamConfig.Beam.Router.Skim.Skimmers$Elm.CountSkimmer(
                name = if (c.hasPathOrNull("name")) c.getString("name") else "count-skimmer",
                skimFileBaseName =
                  if (c.hasPathOrNull("skimFileBaseName")) c.getString("skimFileBaseName") else "skimsCount",
                skimType = if (c.hasPathOrNull("skimType")) c.getString("skimType") else "count-skimmer"
              )
            }
          }
          ...
          object OdSkimmer {
            ...
            def apply(c: com.typesafe.config.Config): BeamConfig.Beam.Router.Skim.Skimmers$Elm.OdSkimmer = {
              BeamConfig.Beam.Router.Skim.Skimmers$Elm.OdSkimmer(
                name = if (c.hasPathOrNull("name")) c.getString("name") else "origin-destination-skimmer",
                skimFileBaseName =
                  if (c.hasPathOrNull("skimFileBaseName")) c.getString("skimFileBaseName") else "skimsOD",
                skimType = if (c.hasPathOrNull("skimType")) c.getString("skimType") else "od-skimmer",
                writeAllModeSkimsForPeakNonPeakPeriodsInterval =
                  if (c.hasPathOrNull("writeAllModeSkimsForPeakNonPeakPeriodsInterval"))
                    c.getInt("writeAllModeSkimsForPeakNonPeakPeriodsInterval")
                  else 0,
          ...
          def apply(c: com.typesafe.config.Config): BeamConfig.Beam.Router.Skim.Skimmers$Elm = {
            BeamConfig.Beam.Router.Skim.Skimmers$Elm(
              count_skimmer =
                if (c.hasPathOrNull("count-skimmer"))
                  scala.Some(BeamConfig.Beam.Router.Skim.Skimmers$Elm.CountSkimmer(c.getConfig("count-skimmer")))
                else None,
              od_skimmer =
                if (c.hasPathOrNull("od-skimmer"))
                  scala.Some(BeamConfig.Beam.Router.Skim.Skimmers$Elm.OdSkimmer(c.getConfig("od-skimmer")))
                else None
            )
          }
        }
        ...
        def apply(c: com.typesafe.config.Config): BeamConfig.Beam.Router.Skim = {
          BeamConfig.Beam.Router.Skim(
            aggregateFunction = if (c.hasPathOrNull("aggregateFunction")) c.getString("aggregateFunction") else "AVG",
            keepKLatestSkims = if (c.hasPathOrNull("keepKLatestSkims")) c.getInt("keepKLatestSkims") else 1,
        ...
        private def $_LBeamConfig_Beam_Router_Skim_Skimmers$Elm(
          cl: com.typesafe.config.ConfigList
        ): scala.List[BeamConfig.Beam.Router.Skim.Skimmers$Elm] = {
          import scala.collection.JavaConverters._
          cl.asScala
            .map(
              cv => BeamConfig.Beam.Router.Skim.Skimmers$Elm(cv.asInstanceOf[com.typesafe.config.ConfigObject].toConfig)
            )
            .toList
        }
      }
    ...
    def apply(c: com.typesafe.config.Config): BeamConfig.Beam = {
      BeamConfig.Beam(
        abstractSkimmer = BeamConfig.Beam.AbstractSkimmer(
          if (c.hasPathOrNull("abstractSkimmer")) c.getConfig("abstractSkimmer")
          else com.typesafe.config.ConfigFactory.parseString("abstractSkimmer{}")
        ),
        agentsim = BeamConfig.Beam.Agentsim(
