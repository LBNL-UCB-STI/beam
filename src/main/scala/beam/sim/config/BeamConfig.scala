// generated by tscfg 0.9.4 on Tue Nov 12 15:47:39 PST 2019
...
object BeamConfig {
  ...
  object Beam {
    ...
    case class Router(
      skim : scala.List[BeamConfig.Beam.Router.Skim$Elm]
    )
    object Router {
      case class Skim$Elm(
        aggregateFunction            : java.lang.String,
        count_skimmer                : scala.Option[BeamConfig.Beam.Router.Skim$Elm.CountSkimmer],
        keepKLatestSkims             : scala.Int,
        name                         : java.lang.String,
        origin_destination_skimmer   : scala.Option[BeamConfig.Beam.Router.Skim$Elm.OriginDestinationSkimmer],
      ...
      object Skim$Elm {
        ...
        object CountSkimmer {
          def apply(c: com.typesafe.config.Config): BeamConfig.Beam.Router.Skim$Elm.CountSkimmer = {
            BeamConfig.Beam.Router.Skim$Elm.CountSkimmer(
              test = if(c.hasPathOrNull("test")) c.getInt("test") else 1
            )
          }
        }
        ...
        object OriginDestinationSkimmer {
          def apply(c: com.typesafe.config.Config): BeamConfig.Beam.Router.Skim$Elm.OriginDestinationSkimmer = {
            BeamConfig.Beam.Router.Skim$Elm.OriginDestinationSkimmer(
              writeAllModeSkimsForPeakNonPeakPeriodsInterval = if(c.hasPathOrNull("writeAllModeSkimsForPeakNonPeakPeriodsInterval")) c.getInt("writeAllModeSkimsForPeakNonPeakPeriodsInterval") else 0,
              writeFullSkimsInterval                         = if(c.hasPathOrNull("writeFullSkimsInterval")) c.getInt("writeFullSkimsInterval") else 0
            )
          }
        ...
        def apply(c: com.typesafe.config.Config): BeamConfig.Beam.Router.Skim$Elm = {
          BeamConfig.Beam.Router.Skim$Elm(
            aggregateFunction            = if(c.hasPathOrNull("aggregateFunction")) c.getString("aggregateFunction") else "LATEST_SKIM",
            count_skimmer                = if(c.hasPathOrNull("count-skimmer")) scala.Some(BeamConfig.Beam.Router.Skim$Elm.CountSkimmer(c.getConfig("count-skimmer"))) else None,
            keepKLatestSkims             = if(c.hasPathOrNull("keepKLatestSkims")) c.getInt("keepKLatestSkims") else 1,
            name                         = if(c.hasPathOrNull("name")) c.getString("name") else "origin-destination-skimmer",
            origin_destination_skimmer   = if(c.hasPathOrNull("origin-destination-skimmer")) scala.Some(BeamConfig.Beam.Router.Skim$Elm.OriginDestinationSkimmer(c.getConfig("origin-destination-skimmer"))) else None,
            skimType                     = if(c.hasPathOrNull("skimType")) c.getString("skimType") else "origin-destination-skimmer",
            writeAggregatedSkimsInterval = if(c.hasPathOrNull("writeAggregatedSkimsInterval")) c.getInt("writeAggregatedSkimsInterval") else 0,
            writeSkimsInterval           = if(c.hasPathOrNull("writeSkimsInterval")) c.getInt("writeSkimsInterval") else 0
          )
        }
      ...
      def apply(c: com.typesafe.config.Config): BeamConfig.Beam.Router = {
        BeamConfig.Beam.Router(
          skim = $_LBeamConfig_Beam_Router_Skim$Elm(c.getList("skim"))
        )
      }
      private def $_LBeamConfig_Beam_Router_Skim$Elm(cl:com.typesafe.config.ConfigList): scala.List[BeamConfig.Beam.Router.Skim$Elm] = {
        import scala.collection.JavaConverters._
        cl.asScala.map(cv => BeamConfig.Beam.Router.Skim$Elm(cv.asInstanceOf[com.typesafe.config.ConfigObject].toConfig)).toList
      }
    }
