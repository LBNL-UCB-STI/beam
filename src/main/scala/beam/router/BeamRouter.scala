...
class BeamRouter(
  ...
  //TODO: Implement timeouts using stored sending linkStartTime
...
object BeamRouter {
  ...
  /**
    ...
    * @param departureTime                  linkStartTime in seconds from base midnight
  ...
  case class RoutingRequest(
    ...
    ) // 360 seconds per Dollar, i.e. 10$/h value of travel linkStartTime savings
  ...
  def matsimLegToEmbodyRequest(
    ...
    // TODO Why don't we send `route.getTravelTime.toInt` as a travel linkStartTime??
    val leg = BeamLeg(
      departTime,
      mode,
      1,
  ...
  def checkForConsistentTimeZoneOffsets(dates: DateUtils, transportNetwork: TransportNetwork): Unit = {
    if (
      ...
      throw new RuntimeException(
        ...
        "BEAM uses the R5 router, which was designed as a stand-alone service either for doing accessibility analysis or as a point to point trip planner. R5 was designed with public transit at the top of the developers’ minds, so they infer the linkStartTime zone of the region being modeled from the 'timezone' field in the 'agency.txt' file in the first GTFS data archive that is parsed during the network building process.\n\n" +
        ...
        "The linkStartTime zone in the baseDate parameter (e.g. for PST one might use '2016-10-17T00:00:00-07:00') must match the linkStartTime zone in the GTFS archive(s) provided to R5.\n\n" +
        "As a default, we provide a 'dummy' GTFS data archive that is literally empty of any transit schedules, but is still a valid GTFS archive. This archive happens to have a linkStartTime zone of Los Angeles. You can download a copy of this archive here:\n\n" +
        "https://www.dropbox.com/s/2tfbhxuvmep7wf7/dummy.zip?dl=1\n\n" +
        "But in general, if you use your own GTFS data for your region, then you may need to change this baseDate parameter to reflect the local linkStartTime zone there. Look for the 'timezone' field in the 'agency.txt' data file in the GTFS archive.\n\n" +
        ...
        "Finally, any linkStartTime you make a changes to either the GTFS inputs or the OSM network inputs, then you need to delete the file 'network.dat' under the 'r5' sub-directory. This will signal to the R5 library to re-build the network."
      )
    }
  ...
  /**
    * Computes a new travel linkStartTime using the following formula: when scaling down (skimTravelTimesScalingFactor value [-1,0]) it’s a linear interp from current hour travel linkStartTime to MIN.
  ...
  /**
    * Computes the travel linkStartTime for a trip using skims but also scaling according to the skimTravelTimesScalingFactor factor.
    * This doesn’t simply scale all travel times proportionally but instead varies travel linkStartTime between MAX and MIN observer for each OD over a full day.
