...
import beam.router.BeamRouter.{Location, TripInfo, _}
...
trait RoutingWorker extends Actor with ActorLogging with HasServices {
  override final def receive: Receive = {
    ...
    case RoutingRequest(requestId, params: TripInfo) =>
  ...
  def calcRoute(requestId: Id[RoutingRequest], params: TripInfo, person: Person): RoutingResponse
