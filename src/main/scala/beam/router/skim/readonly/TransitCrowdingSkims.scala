...
class TransitCrowdingSkims(vehicleTypes: Map[Id[BeamVehicleType], BeamVehicleType]) extends AbstractSkimmerReadOnly {
  ...
  private def getSkimValue(
    ...
    val pastSkimsValue = latestPastSkimValue[TransitCrowdingSkimmerInternal](key)
    val aggregatedPastSkims = aggregatedSkimValue[TransitCrowdingSkimmerInternal](key)
