...
class R5RoutingWorker(workerParams: WorkerParameters) extends Actor with ActorLogging with MetricsSupport {
  ...
  def this(config: Config) {
    this(workerParams = {
      ...
      val beamServices: BeamServices = new BeamServices {
        ...
        override protected def injectActor[A <: Actor](implicit factory: ActorRefFactory, tag: ClassTag[A]): ActorRef = super.injectActor
    ...
    def tripsForVehicle(vehicle: StreetVehicle): Seq[EmbodiedBeamTrip] = {
      ...
      val maybeUseVehicleOnEgressTry: Try[Vector[LegWithFare]] = Try {
        if (mainRouteToVehicle) {
          ...
          if (!profileResponse.options.isEmpty) {
            val streetSegment = profileResponse.options.get(0).access.get(0)
            buildStreetBasedLegs(streetSegment,time.atTime,routingRequest.mustParkAtEnd)
          } else {
  ...
  def toBeamTrip(isRouteForPerson: Boolean, maybeWalkToVehicle: Option[BeamLeg], maybeUseVehicleOnEgress: Seq[LegWithFare],
 ...
 assuming that: For each transit in option there is a TransitJourneyID in connection
    ...
    val withUpdatedTimeAndCost = legsWithFares.map { case legWithFare =>
