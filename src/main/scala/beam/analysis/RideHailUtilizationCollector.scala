...
case class RideInfo(vehicleId: Id[Vehicle], time: Int, startCoord: Coord, endCoord: Coord, numOfPassengers: Int, primaryFuelLevel: Double)
...
class RideHailUtilizationCollector(beamSvc: BeamServices)
  ...
  def handle(pte: PathTraversalEvent): RideInfo = {
    ...
    val vri = RideInfo(pte.vehicleId, pte.time.toInt, startCoord, endCoord, pte.numberOfPassengers, pte.endLegPrimaryFuelLevel)
    rides += vri
    vri
  }
