...
class TransitOccupancyByStopAnalysis extends BasicEventHandler with IterationEndsListener {
  ...
  def writeCSVRows(path: String): Unit = {
    ...
    val analysis = pathTraversalEvents.toList.groupBy(_.vehicleId.toString).map {
