...
class DelayMetricAnalysis @Inject() (eventsManager: EventsManager,
                                     controlerIO: OutputDirectoryHierarchy,
                                     services: BeamServices,scenario: Scenario, beamConfig: BeamConfig) extends BasicEventHandler with LazyLogging {
          ...
          if(linkIds.length > 0){
            for (index <- linkIds.indices) {
              ...
              if (freeFlowDelay > 0){
                ...
                linkAverageDelay(linkId) =DelayInLength((linkTravelsCount.get(linkId).get * cumulativeDelay.get(linkId).get) / cumulativeLength.get(linkId).get, linkTravelsCount.get(linkId).get) //calculate average of link delay for further calculating weighted average
  ...
  def largeset(capacity: Double): Int = {
    bins.reverse.foreach { bin => if (capacity >= bin) return bin }
    0
  }
  ...
  def averageDelayDataset(event: IterationEndsEvent) {
    ...
    val avg = linkAverageDelay.values.map(delayInLength => delayInLength.delay).sum / linkAverageDelay.values.map(delayInLength => delayInLength.length).sum
  ...
  def createDelayCapacityGraph(dataset: CategoryDataset, iterationNumber: Int, fileName:String): Unit = {
    val chart = GraphUtils.createStackedBarChartWithDefaultSettings(dataset, graphTitle, xAxisName, yAxisName, fileName + ".png" , false)
    val graphImageFile = GraphsStatsAgentSimEventsListener.CONTROLLER_IO.getIterationFilename(iterationNumber, fileName + ".png")
  ...
  def createDelayAveragePerKilometerGraph(): Unit = {
    ...
    val chart = GraphUtils.createStackedBarChartWithDefaultSettings(dataset, averageGraphTitle, xAxisAverageGraphName, yAxisAverageGraphName, fileName , false)
