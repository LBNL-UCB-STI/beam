    ...
    alternatives.map{ alt =>
      alt.tripClassifier match {
        case CAR if alt.costEstimate == 0.0 =>
          ...
          val cost = alt.legs.map(_.beamLeg.travelPath.distanceInM).sum * litersPerMeter / drivingCostConfig.defaultLitersPerGallon * drivingCostConfig.defaultPricePerGallon // 3.78 liters per gallon and 3.115 $/gal in CA: http://www.californiagasprices.com/Prices_Nationally.aspx
          BigDecimal(cost)
        case _ =>
          BigDecimal(0)
      }
