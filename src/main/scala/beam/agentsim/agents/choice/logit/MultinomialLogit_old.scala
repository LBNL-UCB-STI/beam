...
import UtilityFunctionParamType.{Intercept, Multiplier}
...
/**
  ...
  * @tparam T
  */
class MultinomialLogit_old[A, T](
...
object MultinomialLogit_old {

  def apply[A, T](utilityFunctionParams: Map[A, Set[UtilityFunctionParam[T]]]): MultinomialLogit_old[A, T] = {
    new MultinomialLogit_old(utilityFunctionParams)
  }
  ...
  ): MultinomialLogit_old[A, T] = {
    new MultinomialLogit_old(reduceInputData(utilityFunctionData), commonUtility)
  }
