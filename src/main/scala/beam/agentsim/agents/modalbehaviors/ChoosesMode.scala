    ...
    case Event(MobilityStatusResponse(newlyAvailableBeamVehicles, triggerId), choosesModeData: ChoosesModeData) =>
      ...
      correctedCurrentTripMode match {
        ...
        case Some(RIDE_HAIL | RIDE_HAIL_POOLED) =>
          ...
          makeRequestWith(withTransit = false, Vector(availablePersonalStreetVehicles.map(_.streetVehicle).filter(c => c.mode.isRideHail || c.mode.isTransit || c.mode.isMassTransit || c.mode.isHovTeleportation).head)) // We need a WALK alternative if RH fails // TODO Maybe instead of WALK, we still give them all others for choosing
          makeRideHailRequest()
    ...
    case FSM.State(
      ...
      val filteredItinerariesForChoice = (choosesModeData.personData.currentTripMode match {
        ...
        case Some(mode) =>
          combinedItinerariesForChoice.filter(_.tripClassifier == mode)
        case _ =>
          combinedItinerariesForChoice.filter(_.tripClassifier != WALK) ++ combinedItinerariesForChoice.filter(_.tripClassifier == WALK).filterNot(_.legs.map(_.beamLeg.travelPath.distanceInM > 4828.03).exists(_ == true))
