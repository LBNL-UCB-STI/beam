...
trait ChoosesMode {
  ...
  def completeChoiceIfReady: PartialFunction[State, State] = {
    case FSM.State(
      ...
      modeChoiceCalculator(filteredItinerariesForChoice, attributesOfIndividual, nextActivity(choosesModeData.personData)) match {
            ...
            case Some(CAV) =>
              ...
              if (cavTripLegs.isEmpty) {
                goto(FinishingModeChoice) using choosesModeData.copy(
                  pendingChosenTrip =
                    Some(EmbodiedBeamTrip(Vector(EmbodiedBeamLeg.dummyLegAt(_currentTick.get, body.id, false))))
                )
              } else {
