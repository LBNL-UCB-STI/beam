...
import beam.agentsim.agents.vehicles.{PassengerSchedule, VehiclePersonId, _}
...
trait DrivesVehicle[T <: BeamAgentData] extends BeamAgent[T] with HasServices {
  ...
  case Event(BecomeDriverSuccess(newPassengerSchedule, assignedVehicle), _)
  =>
...
chainedWhen (AnyState) {
  ...
  beamServices.geo.distInMeters (updatedPassengerSchedule.initialSpacetime.loc, endSpaceTime.loc) > beamServices
  ...
  updatedPassengerSchedule.schedule.foreach {
  legAndManifest =>
  legAndManifest._2.riders.foreach {
  rider =>
  ...
  // XXXX (VR): from vehicle's CancelReservationcase Event(RemovePassengerFromTrip(vehiclePersonId),_)=>
  ...
  context.system.eventStream.publish (new PathTraversalEvent (theTick, _currentVehicleUnderControl
  .get.id,
  ...
  .addPassenger (req
  .passengerVehiclePersonId.vehicleId).flatMap ( {
  _ =>
