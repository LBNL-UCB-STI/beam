...
object PersonAgent {
  ...
  def props(
    ...
    Props(new PersonAgent(scheduler, services, modeChoiceCalculator, transportNetwork, router, rideHailingManager, eventsManager, personId, plan, humanBodyVehicleId, parkingManager))
  }
  ...
  case class LiterallyDrivingData(delegate: DrivingData, legEndsAt: Double) extends DrivingData { // sorry
    ...
    def withPassengerSchedule(newPassengerSchedule: PassengerSchedule): DrivingData = LiterallyDrivingData(
      delegate
        .withPassengerSchedule(newPassengerSchedule), legEndsAt
    )

    def withCurrentLegPassengerScheduleIndex(currentLegPassengerScheduleIndex: Int) = LiterallyDrivingData(
      delegate
        .withCurrentLegPassengerScheduleIndex(currentLegPassengerScheduleIndex), legEndsAt
    )
  ...
  case class BasePersonData(
    ...
    override def withPassengerSchedule(
      newPassengerSchedule: PassengerSchedule)
    : DrivingData = copy(passengerSchedule = newPassengerSchedule)
...
class PersonAgent(
...
) extends DrivesVehicle[PersonData] with ChoosesMode with ChoosesParking[PersonData] with Stash {
  ...
  def activityOrMessage(ind: Int, msg: String): Either[String, Activity] = {
    if (ind < 0 || ind >= _experiencedBeamPlan.activities.length) Left(msg) else Right(
      ...
      eventsManager
        .processEvent(
          new ActivityEndEvent(
            tick, id, currentActivity(data).getLinkId, currentActivity(data)
              .getFacilityId, currentActivity(data).getType
          )
        )
      ...
      eventsManager
        .processEvent(
          new PersonDepartureEvent(
            tick, id, currentActivity(data).getLinkId, currentTrip.tripClassifier
              .value
          )
        )
      holdTickAndTriggerId(tick, triggerId)
  ...
  when(Waiting) {
     ...
     */
    case Event(TriggerWithId(NotifyLegStartTrigger(_, beamLeg), triggerId), BasePersonData(_, _, currentLeg :: _, currentVehicle, _, _, _, _, _)) if currentLeg
      .beamVehicleId == currentVehicle.head =>
    ...
    case Event(TriggerWithId(NotifyLegStartTrigger(tick, beamLeg), triggerId), data@BasePersonData(_, _, currentLeg :: _, currentVehicle, _, _, _, _, _)) =>
      ...
      goto(Moving) replying CompletionNotice(triggerId) using data
        .copy(currentVehicle = currentLeg.beamVehicleId +: currentVehicle)
  }
  ...
  when(Moving) {
     ...
     */
    case Event(TriggerWithId(NotifyLegEndTrigger(_, beamLeg), triggerId), data@BasePersonData(_, _, currentLeg :: restOfCurrentTrip, currentVehicle, _, _, _, _, _)) if restOfCurrentTrip
      .head.beamVehicleId == currentVehicle.head =>
  ...
  when(ProcessingNextLegOrStartActivity, stateTimeout = Duration.Zero) {
    ...
    case Event(StateTimeout, data@BasePersonData(_, _, nextLeg :: restOfCurrentTrip, currentVehicle, _, _, _, _, _)) if nextLeg
      .asDriver =>
      val (tick, triggerId) = releaseTickAndTriggerId()
          ...
          vehicle.becomeDriver(self).fold(
            fa =>
              throw new RuntimeException(
                s"I attempted to become driver of vehicle $id but driver ${
                  vehicle.driver
                    .get
                } already assigned."
              ),
            fb => {
              eventsManager
                .processEvent(new PersonEntersVehicleEvent(tick, Id.createPersonId(id), nextLeg.beamVehicleId))
            }
          )
    ...
    case Event(StateTimeout, BasePersonData(_, _, nextLeg :: tailOfCurrentTrip, _, _, _, _, _, _)) if nextLeg.beamLeg
      .mode.isTransit() =>
      ...
      val resRequest = new ReservationRequest(
        legSegment.head.beamLeg, legSegment.last
          .beamLeg, VehiclePersonId(legSegment.head.beamVehicleId, id)
      )
          ...
          eventsManager
            .processEvent(
              new TeleportationArrivalEvent(
                tick, id, currentTrip.legs
                  .map(l => l.beamLeg.travelPath.distanceInM).sum
              )
            )
          assert(activity.getLinkId != null)
          eventsManager
            .processEvent(new PersonArrivalEvent(tick, id, activity.getLinkId, currentTrip.tripClassifier.value))
          eventsManager
            .processEvent(
              new ActivityStartEvent(
                tick, id, activity.getLinkId, activity.getFacilityId, activity
                  .getType
              )
            )
