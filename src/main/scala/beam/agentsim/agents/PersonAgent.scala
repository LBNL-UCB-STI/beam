...
object PersonAgent {
  ...
  def props(scheduler: ActorRef, services: BeamServices, modeChoiceCalculator: ModeChoiceCalculator, transportNetwork: TransportNetwork, router: ActorRef, rideHailingManager: ActorRef, parkingManager: ActorRef, eventsManager: EventsManager, personId: Id[PersonAgent], household: Household, plan: Plan,
...
class PersonAgent(val scheduler: ActorRef, val beamServices: BeamServices, val modeChoiceCalculator: ModeChoiceCalculator,
  ...
  when(ReadyToChooseParking, stateTimeout = Duration.Zero) {
    case Event(StateTimeout, data@BasePersonData(_,_,currentLeg::theRestOfCurrentTrip,_,_,_,_,_,_)) =>
      ...
      goto(ChoosingParkingSpot) using ChoosesParkingData(data.asInstanceOf[BasePersonData].copy(restOfCurrentTrip=theRestOfCurrentTrip))
  }
