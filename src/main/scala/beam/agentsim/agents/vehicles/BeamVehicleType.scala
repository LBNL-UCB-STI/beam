...
sealed abstract class BeamVehicleType(val idString: String) extends EnumEntry {
  ...
  lazy val MatsimVehicleType: VehicleType =
    VehicleUtils.getFactory.createVehicleType(Id.create(this.getClass.getName, classOf[VehicleType]))
  ...
  def createMatsimVehicle[T](id: Id[T]): Vehicle = {
    id match {
      case personId: Id[Person] => VehicleUtils.getFactory.createVehicle(createId(personId), MatsimVehicleType)
      case vehicleId: Id[Vehicle] => VehicleUtils.getFactory.createVehicle(vehicleId, MatsimVehicleType)
    }
  }
