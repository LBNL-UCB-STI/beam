...
object CarVehicle extends BeamVehicleObject {
  ...
  def props(beamServices: BeamServices, eventsManager: EventsManager, vehicleId: Id[Vehicle], data: CarVehicleData, powerTrain: Powertrain,
            initialMatsimVehicle: Vehicle, initialMatsimAttributes: Attributes) = {
    Props(new CarVehicle(beamServices, eventsManager, vehicleId, data, powerTrain, initialMatsimVehicle, initialMatsimAttributes))
  }
  ...
  override def props(beamServices: BeamServices, eventsManager: EventsManager, vehicleId: Id[Vehicle], matSimVehicle: Vehicle, powertrain: Powertrain): Props = {
    ...
    props(beamServices, eventsManager, vehicleId, CarVehicleData(), powertrain, matSimVehicle, new Attributes())
  }
  ...
  def props(beamServices: BeamServices, eventsManager: EventsManager, matSimVehicle: Vehicle, powertrain: Powertrain): Props = {
    props(beamServices, eventsManager, matSimVehicle.getId, CarVehicleData(), powertrain, matSimVehicle, new Attributes())
  }
