...
object RideHailManager {
  ...
  val INITIAL_RIDE_HAIL_LOCATION_HOME_HISTORICAL = "HOME_HISTORICAL"
...
class RideHailManager(
  ...
  // We will be removing one by one from this list from the end - that's why it is `ListBuffer` (constant time to remove last element)
  val shouldRepositionInTheBeginning: mutable.ListBuffer[Id[Vehicle]] = {
    val shouldReposition: Seq[Id[Vehicle]] = rhs.getRideHailUtilization.notMovedAtAll.toSeq ++ rhs.getRideHailUtilization.movedWithoutPassenger.toSeq
    mutable.ListBuffer(shouldReposition :_*)
  }
  val activities: Array[Activity] = RandomUtils.shuffle(scenario.getPopulation.getPersons.values().asInstanceOf[java.util.Collection[Person]].asScala
    .flatMap { p =>
      val acts = p.getSelectedPlan.getPlanElements.asScala.collect {
  ...
  // This is TreeMap because we want to remove used coord fast (logN). The index is generated by random number generator `rand`
  val possiblePlaceToReposition: mutable.HashMap[Int, Location] = ProfilingUtils.timed("Build possiblePlaceToReposition", log.info){
    val coords = rhs.getRideHailUtilization.movedWithPassengers.map(_.startCoord)
    val distinctCoords = coords.distinct
    val idxToCoord = RandomUtils.shuffle(distinctCoords, rand)
      .zipWithIndex.map { case (coord, idx) => (idx, coord)}.toArray
    log.info(s"coords size: ${coords.size}, distinctCoords size: ${distinctCoords.size} idxToCoord size: ${idxToCoord.length}")
    mutable.HashMap[Int, Coord](idxToCoord :_*)
  }

  log.info(s"activities: ${activities.size}")
  log.info(s"shouldRepositionInTheBeginning: ${shouldRepositionInTheBeginning.size}")
  log.info(s"possiblePlaceToReposition: ${possiblePlaceToReposition.size}")
  ...
  log.info("After initializing the locations")
  log.info(s"shouldRepositionInTheBeginning: ${shouldRepositionInTheBeginning.size}")
  log.info(s"possiblePlaceToReposition: ${possiblePlaceToReposition.size}")
  ...
  val maybeTheStashMethod: Option[Method] = Try(this.getClass.getDeclaredMethod("akka$actor$StashSupport$$theStash")) match {
  ...
  def getRideInitLocation(person: Person): Location = {
    val rideInitialLocation: Location =
      beamServices.beamConfig.beam.agentsim.agents.rideHail.initialization.procedural.initialLocation.name match {
        ...
        case RideHailManager.INITIAL_RIDE_HAIL_LOCATION_HOME | RideHailManager.INITIAL_RIDE_HAIL_LOCATION_HOME_HISTORICAL =>
          if (beamServices.beamConfig.beam.agentsim.agents.rideHail.initialization.procedural.initialLocation.name == RideHailManager.INITIAL_RIDE_HAIL_LOCATION_HOME ||
            beamServices.iterationNumber == 0) {
            ...
            if (shouldRepositionInTheBeginning.nonEmpty) {
              shouldRepositionInTheBeginning.remove(shouldRepositionInTheBeginning.size - 1)
              val nextActivity = activities(rand.nextInt(activities.length))
              nextActivity.getCoord
            }
            else {
              // We want to pick random index uniformly distributed
              val keys = possiblePlaceToReposition.keys.toArray
              val nextIdx = rand.nextInt(keys.length)
              val idxToRemove = keys(nextIdx)
              // Remove that index so it won't be used anymore (logN)
              possiblePlaceToReposition.remove(idxToRemove) match {
                case Some(coord) => coord
                case None => throw new IllegalArgumentException(s"nextIdx: $nextIdx, keys.size: ${keys.size}")
              }
            }
          }
