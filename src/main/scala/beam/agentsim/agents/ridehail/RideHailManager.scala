...
class RideHailManager(
  ...
  val shouldRepositionInTheBeginning: mutable.ListBuffer[Id[Vehicle]] = {
    val shouldReposition
      : Seq[Id[Vehicle]] = rhs.getRideHailUtilization.notMovedAtAll.toSeq ++ rhs.getRideHailUtilization.movedWithoutPassenger.toSeq
    mutable.ListBuffer(shouldReposition: _*)
  }

  val activities: Array[Activity] = RandomUtils
    .shuffle(
      scenario.getPopulation.getPersons
        .values()
        .asInstanceOf[java.util.Collection[Person]]
        .asScala
        .flatMap { p =>
  ...
  val possiblePlaceToReposition: mutable.HashMap[Int, Location] =
    ProfilingUtils.timed("Build possiblePlaceToReposition", log.info) {
      val coords = rhs.getRideHailUtilization.rides.withFilter(x => x.numOfPassengers > 0).map(_.startCoord)
      val idxToCoord = RandomUtils.shuffle(coords, rand).zipWithIndex.map { case (coord, idx) => (idx, coord) }.toArray
      log.info(
        s"coords size: ${coords.size}, distinctCoords size: ${coords.distinct.size} idxToCoord size: ${idxToCoord.length}"
      )
  ...
  val msgBefore =
    s"""
      |Before initializing the locations
      |shouldRepositionInTheBeginning: ${shouldRepositionInTheBeginning.size}
      |possiblePlaceToReposition: ${possiblePlaceToReposition.size}""".stripMargin
  ...
  val msgAfter =
    s"""
       |After initializing the locations:
       |shouldRepositionInTheBeginning: ${shouldRepositionInTheBeginning.size}
       |possiblePlaceToReposition: ${possiblePlaceToReposition.size}""".stripMargin
    ...
    val rideInitialLocation: Location =
      beamServices.beamConfig.beam.agentsim.agents.rideHail.initialization.procedural.initialLocation.name match {
        ...
        case RideHailManager.INITIAL_RIDE_HAIL_LOCATION_HOME |
            RideHailManager.INITIAL_RIDE_HAIL_LOCATION_HOME_HISTORICAL =>
          if (beamServices.beamConfig.beam.agentsim.agents.rideHail.initialization.procedural.initialLocation.name == RideHailManager.INITIAL_RIDE_HAIL_LOCATION_HOME ||
            ...
            if (shouldRepositionInTheBeginning.nonEmpty) {
              ...
              // Remove that index so it won't be used anymore
