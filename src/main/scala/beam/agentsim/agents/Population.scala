...
class Population(val scenario: Scenario,
  ...
  private def initHouseholds(iterId: Option[String] = None): Unit = {
    ...
    val houseHoldsInitialized = Future.sequence(
      scenario.getHouseholds.getHouseholds.values().asScala.map { household =>
        ...
        if (scenario.getHouseholds.getHouseholdAttributes.getAttribute(
              household.getId.toString.toLowerCase(),
              "homecoordy") == null) {
          log.error(
    ...
    log.info(
      s"Initialized ${scenario.getHouseholds.getHouseholds.size} households")
  }

}

object Population {
  def props(scenario: Scenario,
