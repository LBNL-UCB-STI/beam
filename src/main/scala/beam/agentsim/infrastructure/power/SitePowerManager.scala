...
object SitePowerManager {
  ...
  def constructSitePowerKey(
    ...
    createSiteId(s"site-$reservedFor-$geoId-$parkingType-$chargingLevel")
  }

  def createSiteId(siteId: String): Id[SitePowerManager] = Id.create(siteId, classOf[SitePowerManager])
}
