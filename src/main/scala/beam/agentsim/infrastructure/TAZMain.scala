...
class TAZTreeMap(val tazQuadTree: QuadTree[TAZ]) {
  ...
  def getTAZInRadius(x: Double,
                     y: Double,
                     radius: Double): util.Collection[TAZ] = {
    // TODO: is this enough precise, or we want to get the exact TAZ where the coordinate is located?
...
object TAZTreeMap {
  ...
  private def initQuadTreeFromShapeFile(
    ...
    val tazQuadTree: QuadTree[TAZ] = new QuadTree[TAZ](quadTreeBounds.minx,
                                                       quadTreeBounds.miny,
                                                       quadTreeBounds.maxx,
                                                       quadTreeBounds.maxy)
    ...
    for (f <- features.asScala) {
      f.getDefaultGeometry match {
        case g: Geometry =>
          val taz = new TAZ(f.getAttribute(tazIDFieldName).asInstanceOf[String],
  ...
  private def quadTreeExtentFromShapeFile(
      features: util.Collection[SimpleFeature]): QuadTreeBounds = {
  ...
  private def readCsvFile(filePath: String): Seq[CsvTaz] = {
    ...
    try {
      mapReader = new CsvMapReader(readerFromFile(filePath),
  ...
  def featureToCsvTaz(f: SimpleFeature, tazIDFieldName: String) = {
    f.getDefaultGeometry match {
      case g: Geometry =>
        Some(
          CsvTaz(f.getAttribute(tazIDFieldName).toString,
                 g.getCoordinate.x,
                 g.getCoordinate.y))
      case _ => None
    }
  ...
  def shapeFileToCsv(shapeFilePath: String,
    ...
    lazy val utm2Wgs: GeotoolsTransformation =
      new GeotoolsTransformation("utm", "EPSG:26910")
    ...
    try {
      ...
      val (repeatedTaz, nonRepeatedMap) =
        groupedTazs.partition(i => i._2.length > 1)
      ...
      val targetCrsNum =
        CRS.lookupEpsgCode(shapeFileReader.getCoordinateSystem, true)
      ...
      for (t <- allNonRepeatedTaz) {
        ...
        //
        val transFormedCoord: Coord =
          wGSConverter.wgs2Utm.transform(new Coord(t.coordX, t.coordY))
        val tcoord = utm2Wgs.transform(
          new Coord(transFormedCoord.getX, transFormedCoord.getY))
  ...
  private def getProcessors: Array[CellProcessor] = {
    Array[CellProcessor](new UniqueHashCode(), // Id (must be unique)
                         new NotNull(), // Coord X
                         new NotNull()) // Coord Y
  }
  ...
  private def clearRepeatedTaz(
      groupedRepeatedTaz: Map[String, Array[CsvTaz]]): Array[CsvTaz] = {
  ...
  private def closestToPoint(referencePoint: Double,
                             elems: Array[CsvTaz]): CsvTaz = {
    elems.reduce { (a, b) =>
...
case class QuadTreeBounds(minx: Double,
                          miny: Double,
                          maxx: Double,
                          maxy: Double)
