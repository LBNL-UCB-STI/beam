...
package object agentsim {
  ...
  implicit def personAgentId2PersonId(id: Id[PersonAgent]): Id[Person] =
    Id.createPersonId(id)

  implicit def vehicleId2BeamVehicleId(id: Id[Vehicle]): Id[BeamVehicle] =
    Id.create(id, classOf[BeamVehicle])

  implicit def beamVehicleId2VehicleId(id: Id[BeamVehicle]): Id[Vehicle] =
  ...
  implicit def beamVehicleMaptoMatsimVehicleMap(
    ...
    beamVehicleMap.map({
      case (vid, veh) => (Id.createVehicleId(vid), veh.matSimVehicle)
    })
  }
      ...
      .map({
        case (vid, veh) =>
          (Id.create(vid, classOf[BeamVehicle]),
           new BeamVehicle(
             Powertrain.PowertrainFromMilesPerGallon(
               veh.getType.getEngineInformation.getGasConsumption),
             veh,
             None,
             BeamVehicleType.Car,
             None,
  ...
  implicit def personId2RideHailAgentId(id: Id[Person]): Id[RideHailAgent] = {
    Id.create(s"${RideHailAgent.idPrefix}${prefixStrip(id)}",
