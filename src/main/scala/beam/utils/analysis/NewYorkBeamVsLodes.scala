...
object NewYorkBeamVsLodes {
  private case class GeoAttribute(state: String, county: String, tractCode: String, blockGroup: String, censusBlockGroup: String) extends Attributes
  ...
  def main(args: Array[String]): Unit = {
    ...
    val quadTree = new QuadTree[(GeoAttribute, MultiPolygon)](envelope.getMinX, envelope.getMinY, envelope.getMaxX, envelope.getMaxY)
    allFeatures.foreach {
      case (k, v) =>
        quadTree.put(v.getCentroid.getX, v.getCentroid.getY, ((k, v)))
    }
    val homeToWork = NewYorkHomeWorkLocationAnalysis.getHomeWorkLocations("test/input/newyork/generic_scenario/1049k-NYC-related/plans.csv.gz", isUtmCoord = false)
    var originIsNotInside: Int = 0
    var destIsNotInside: Int = 0

    homeToWork.foreach { case ((origin, destination), cnt) =>
      val (originAttrib, originPolygon) = quadTree.getClosest(origin.getX, origin.getY)
      val (destAttrib, destPolygon) = quadTree.getClosest(destination.getX, destination.getY)
...
//      println(s"destAttrib: $destAttrib, isFoundDestInside: $isFoundDestInside")
    ...
    println(s"originIsNotInside: $originIsNotInside")
    println(s"destIsNotInside: $destIsNotInside")
