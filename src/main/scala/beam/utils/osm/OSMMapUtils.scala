...
object OSMMapUtils {
  ...
  def showInfo(osm: OSM): Unit = {
    ...
    wayTypes.toSeq.sortBy { case (_, count) => count }.reverse.take(5).foreach { case (wayType, count) =>
      println(s"$wayType : $count")
    }
  }
  ...
  def getWayLen(osm: OSM, way: Way): Double = {
    val (_, distance) = way.nodes.tail.foldLeft((way.nodes.head, 0.0)) { case ((node1, totalDistance), node2) =>
      (node2, getDistance(osm, node1, node2) + totalDistance)
    }
  ...
  def writeOSMInfo(osm: OSM, outputCSVFilePath: String): Unit = {
    ...
    osm.ways.asScala.foreach { case (wayId, way) =>
      csvWriter.writeRow(IndexedSeq(wayId, way.nodes.length, getWayLen(osm, way)))
    }
  ...
  def removeShortWays(
    ...
    osm.relations.asScala.foreach { case (relationId, relation) =>
      copy.writeRelation(relationId, relation)
    }
  ...
  def main(args: Array[String]): Unit = {
    ...
    val roadsToProcess = Set[String]("residential")
    val copy = removeShortWays(map, roadsToProcess, 30.0)
    showInfo(copy)
    ...
    def choosePercentOfResidentialAndFootway(way: Way): Boolean = {
      if (
        way.tags == null || !way.tags.asScala
          .exists(tag => tag.key == "highway" && (tag.value == "residential" || tag.value == "footway"))
      ) {
        true
      } else {
