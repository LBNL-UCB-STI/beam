...
import beam.utils.CsvFileUtils.readCsvFileByLineToMap
...
object BeamVehicleUtils {
  ...
  def readFuelTypeFile(filePath: String): scala.collection.Map[FuelType, Double] = {
    readCsvFileByLineToMap[FuelType, Double](filePath) { line =>
      ...
      (fuelType, priceInDollarsPerMJoule)
    }
  }

  def readBeamVehicleTypeFile(filePath: String): Map[Id[BeamVehicleType], BeamVehicleType] = {
    readCsvFileByLineToMap[Id[BeamVehicleType], BeamVehicleType](filePath) { line =>
