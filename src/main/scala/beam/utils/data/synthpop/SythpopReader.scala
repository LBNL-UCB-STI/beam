...
class SythpopReader(val houseHoldsWithPeople: Seq[(File, File)]){
  def read(): Map[Household, Seq[Person]] = {
    val (_, allHouseholdsWithPeople) = houseHoldsWithPeople.foldLeft((0, Map[Household, Seq[Person]]())) { case ((totalNumberOfPeople, resultMap), (hhFile, personFile) ) =>
      val households =
        new HouseholdReader(hhFile.getAbsolutePath).read().groupBy(x => x.id).map {
          case (hhId, xs) => hhId -> xs.head
        }

      val householdIdToPersons = new PopulationReader(personFile.getAbsolutePath).read().zipWithIndex.groupBy { case (p, _) => p.householdId }
      val householdWithPersons = householdIdToPersons.map {
        case (hhId, persons) =>
          ...
          val updatedPersons = persons.map { case (p, idx) =>
...
object SythpopReader extends StrictLogging {
  def apply(folderToScan: String): SythpopReader = {
    ...
    hhWithPeople.foreach { case (hh, people) =>
