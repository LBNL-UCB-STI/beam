...
import beam.utils.data.synthpop.models.Models.PumaGeoId
import com.typesafe.scalalogging.StrictLogging
import org.apache.commons.math3.random.MersenneTwister

trait WorkDestinationGenerator {
  def next(homeLocation: PumaGeoId, income: Double): Option[String]
}
...
class RandomWorkDestinationGenerator(val pathToCTPPData: PathToData, val randomSeed: Int)
  ...
  override def next(homeLocation: PumaGeoId, income: Double): Option[String] = {
    householdGeoIdToIncomeOD.get(homeLocation.asUniqueKey) match {
      case Some(xs) =>
        ...
        ODSampler.sample(incomeInRange, rndGen).map(_.destination)
      case None =>
        logger.info(
