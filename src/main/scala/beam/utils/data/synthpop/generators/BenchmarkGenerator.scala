...
class BenchmarkGenerator(
  val odList: Seq[OD[MeansOfTransportation]],
...
object BenchmarkGenerator {
  ...
  def apply(pathToCTPPFolder: String, pathToTazShapeFile: String, pathToOsmMap: String): BenchmarkGenerator = {
    val od =
      new MeansOfTransportationTableReader(PathToData(pathToCTPPFolder), ResidenceToWorkplaceFlowGeography.`TAZ To TAZ`)
        .read()
    val tazGeoIdToGeom: Map[TazGeoId, Geometry] =
  ...
  def main(args: Array[String]): Unit = {
    ...
    val bg = BenchmarkGenerator.apply(pathToCTTPData, pathToTazShapeFile, pathToOSMFile)
