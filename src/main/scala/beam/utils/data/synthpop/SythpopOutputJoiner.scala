...
object SythpopOutputUnion {
  def main(args: Array[String]): Unit = {
    ...
    householdFiles.map { f =>
      val name = f.getName.replaceAll("household_", "")
      name -> f
    }.sortBy { case (name, _) => name }
      .map(_._2)

    val peopleFiles = new File(folderToScan).listFiles(new FileFilter {
      override def accept(pathname: File): Boolean = {
    ...
    val householdCsvWriter = new CsvWriter(outputFolder + "/all_households.csv", Vector("NP","VEH", "HINCP", "hh_children", "workers", "state", "county", "tract","block group", "id"))
    ...
    try {
      ...
      householdFiles.zip(peopleFiles).foreach { case (householdFile, peopleFile) =>
        ...
        val households =
          new HouseholdReader(householdFile.getAbsolutePath).read().groupBy(x => x.id).map { case (hhId, xs) => hhId -> xs.head }
        ...
        householdWithPersons.foreach { case (household, persons) =>
          ...
          householdCsvWriter.write(household.numOfPersons, household.numOfVehicles, household.income, hh_children_val , household.numOfWorkers,
            household.geoId.state, household.geoId.county, household.geoId.tract, household.geoId.blockGroup, household.id)
