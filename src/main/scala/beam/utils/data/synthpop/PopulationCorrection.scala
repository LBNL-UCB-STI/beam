...
object PopulationCorrection extends StrictLogging {
  ...
  def adjust(
    ...
    val population = if (keepOnlyElderThan16Years) {
      ...
      logger.info(
        s"""After filtering them got ${elderThan16Years.size} households with ${elderThan16Years.values
          .map(x => x.size)
          .sum} people.
      ...
      input.filter { case (_, persons) => persons.nonEmpty }.toMap
    }

    if (shouldRemoveNonWorkers) {
      val tempFinalResult = population.foldLeft(Map[Models.Household, Seq[Models.Person]]()) {
      ...
      val removedEmptyHh = population.size - tempFinalResult.size
      val removedNonWorkers = population.map(x => x._2.size).sum - tempFinalResult.values.map(x => x.size).sum
      logger.info(
