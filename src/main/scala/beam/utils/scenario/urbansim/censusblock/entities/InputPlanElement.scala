...
object InputPlanElement extends EntityTransformer[InputPlanElement] {
  ...
  override def transform(m: util.Map[String, String]): InputPlanElement = {
    val tripId = getIfNotNull(m, "trip_id").split("\\.").apply(0)
    ...
    InputPlanElement(tripId, personId, planElementIndex, activityElement, tripMode, activityType, xWgs, yWgs, departureTime)
  }
}
