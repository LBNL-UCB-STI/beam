...
//    readAs[PlanElement](path, toPlanElement, filter)
  ...
  private def toPlanElement(leg: Leg, plan: Plan, planIdx: Int, person: Person, planElementIdx: Int): PlanElement =
    PlanElement(
      ...
      legExpectedTravelTime = Option(leg.getAttributes.getAttribute("trip_dur_min")).map(_.toString).map(_.toDouble),
      legExpectedCost = Option(leg.getAttributes.getAttribute("trip_cost_dollars")).map(_.toString).map(_.toDouble),
