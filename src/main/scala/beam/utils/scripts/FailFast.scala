...
object FailFast extends LazyLogging {
  ...
  def run(beamServices: BeamServices): Unit = {
     ...
     */
    if (c.matsim.modules.strategy.planSelectorForRemoval.equals("tryToKeepOneOfEachClass") &&
        !c.beam.agentsim.agents.modalBehaviors.modeChoiceClass.equals("ModeChoiceLCCM")) {
     ...
     */
    if (c.beam.agentsim.agents.rideHail.allocationManager.name
          .equals("POOLING_ALONSO_MORA") && c.beam.agentsim.agents.rideHail.allocationManager.requestBufferTimeoutInSeconds == 0) {
      throw new RuntimeException(
    ...
    } else if (c.beam.agentsim.agents.rideHail.allocationManager.name
                 .equals("DEFAULT_MANAGER") && c.beam.agentsim.agents.rideHail.allocationManager.requestBufferTimeoutInSeconds > 0) {
      throw new RuntimeException(
    ...
    if (c.beam.physsim.writeRouteHistoryInterval < 0) {
      throw new RuntimeException(
        "Wrong value of Route History file writing iteration"
      )
    }

    if(c.beam.beamskimmer.collectFullCarSkimsInterval > 0 && c.beam.beamskimmer.collectFullCarSkimsInterval%c.beam.beamskimmer.writeObservedSkimsInterval != 0) {
      throw new RuntimeException(
        s"collectFullCarSkimsInterval won't be written because it doesn't coincide with collectFullCarSkimsInterval: ${c.beam.beamskimmer.collectFullCarSkimsInterval} % ${c.beam.beamskimmer.collectFullCarSkimsInterval} != 0"
      )
    }
  }
}
