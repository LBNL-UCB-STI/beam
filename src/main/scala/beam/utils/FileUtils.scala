...
object FileUtils extends LazyLogging {
  ...
  def setConfigOutputFile(beamConfig: BeamConfig,
  ...
  def getConfigOutputFile(outputDirectoryBasePath: String,
    ...
    val outputDir = Paths
      .get(
        outputDirectoryBasePath + File.separator + simulationName + "_" + optionalSuffix)
      .toFile
    logger.debug(s"Beam output directory is: ${outputDir.getAbsolutePath}")
    outputDir.mkdir()
  ...
  def getOptionalOutputPathSuffix(
      addTimestampToOutputDirectory: Boolean): String = {
  ...
  def downloadFile(source: String): Unit = {
    downloadFile(source,
                 Paths.get(getTempDirectoryPath, getName(source)).toString)
  }
