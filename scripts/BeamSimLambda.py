...
runcmd:
  ...
  - java -jar beam.jar --config test/input/$INPUTS/beam.conf
  - sleep 10s
...
def deploy(branch, build_id, input, shutdown_wait):
    script = initscript.replace('$REGION',os.environ['REGION']).replace('$BRANCH',branch).replace('$BUILD_ID', build_id).replace('$INPUTS', input).replace('$UID', uid).replace('$SHUTDOWN_WAIT', shutdown_wait)
