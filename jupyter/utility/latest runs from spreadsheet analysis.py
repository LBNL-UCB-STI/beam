...
def simulation_to_project_map(simulation_name):
    if 'sfbay' in simulation_name and '503440616atberkeley_edu' in simulation_name:
        return "Xuan sf-bay runs"
    if '503440616atberkeley_edu' in simulation_name:
        return "Xuan runs"
    if 'new-york' in simulation_name:
        return "NYC"
    if 'freight' in simulation_name:
        return "Freight"
    if 'gemini' in simulation_name:
        return "Gemini"
    if 'micro-mobility' in simulation_name or 'micromobility' in simulation_name:
        return "Micro-Mobility"
    if 'shared' in simulation_name:
        return "Shared Fleet"
    if 'profiling' in simulation_name:
        return "CPU profiling"
    if 'omx-skim' in simulation_name and 'zneedellatgmail' in simulation_name:
...
def get_project(row):
    ...
    simulation_name = f"{branch_owner} | {run_name}".lower()

    project_name = simulation_to_project_map(simulation_name)
    simulations_group_in_project = group_to_project.get(project_name, set())
    simulations_group_in_project.add(simulation_name)
    group_to_project[project_name] = simulations_group_in_project
...
print(f"there are {len(list_of_all_projects)} projects ({len(group_to_project[other_name])} runs classified as '{other_name}'):")
...
# adding a 'Total' row to check that sum of every column is expected
