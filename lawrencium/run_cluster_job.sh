...
if [[ "${!BATCH_MODE_SENTINEL:-submit}" != "batch" ]]; then
  ...
  SBATCH_EXPORTS="ALL,BEAM_BASE_DIR=$BEAM_BASE_DIR,JOB_LOG_FILE_PATH=$JOB_LOG_FILE_PATH,LINK_TO_JOB_LOG_FILE=$LINK_TO_JOB_LOG_FILE,AKKA_PORT=$AKKA_PORT,BEAM_IMAGE_MODE=${BEAM_IMAGE_MODE},BEAM_CONFIG=$BEAM_CONFIG,DOCKER_IMAGE_NAME=${DOCKER_IMAGE_NAME:-},IMAGE_TAG=${IMAGE_TAG:-},PREBUILT_SIF_PATH=${PREBUILT_SIF_PATH:-},LAUNCHER_SMOKE_ONLY=${LAUNCHER_SMOKE_ONLY}"
