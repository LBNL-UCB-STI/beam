...
def lambda_handler(event, context):
    ...
    channel_id=safe_value_with_default(slack_ids_as_dict, email, None)
    if channel_id==None:
        logger.info("channel id can not be determined")
        return
    data = {
        "channel": channel_id,
        "text": payload,
    ...
    conn = http.client.HTTPSConnection(os.environ['SLACK_API_URL'])
    conn.request('POST', os.environ['ENDPOINT'], json.dumps(data), headers)
