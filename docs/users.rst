 ...
 * directly run script from `beam/src/main/scala/beam/utils/beamToVia/EventsByVehicleMode.scala`
 * run script by gradle task::

        gradle execute -PmainClass=beam.utils.beamToVia.EventsByVehicleMode -PappArgs="[<parameters>]"
...
Example: `gradle execute -PmainClass=beam.utils.beamToVia.EventsByVehicleMode -PappArgs="['BEAM events file path', 'output file path', 'car,bus', '1']" -PmaxRAM=16g`
...
Example: `gradle execute -PmainClass=beam.utils.beamToVia.EventsByVehicleMode -PappArgs="['BEAM events file path', 'output file path', 'car,bus', '0.2', 'path to physSimNetwork.xml', '548966', '4179000', '5000']" -PmaxRAM=16g`
...
Also there are bunch of various scripts to create MATSim files for more complex visualizations like traking a person or group of vehicles.
Those scripts located in `beam/src/main/scala/beam/utils/beamToVia/apps` and `beam/src/main/scala/beam/utils/beamToVia/appsForVisualizations`.
There are no way to run them other than direct call and those scripts will require some tuning and/or rewriting.
