...
We can configure beam to run from previous state. This configuration can be controlled by following parameters in config file::
...
We can configure each param as following

- `beam.warmStart.type`: This parameter control type of warm start. We have following options for this param.

 - **disabled** - completely disable warm start
 - **full** - All warm start files has been used for initialising warm start
...
- `beam.warmStart.path`: This property consist path of warm starts files from some previous run. We can give path of zip file on s3 or local dir

- `beam.warmStart.samplePopulationIntegerFlag`: This parameter control if population sampling needed or not. For full warmstart run we can not change sampling

- `beam.warmStart.prepareData`: This parameter control if we have to prepare warmstart archive for distribution. If true then beam write following file in archive.

   Root level files

      output_personAttributes.xml.gz, population.csv.gz,households.csv.gz,vehicles.csv.gz

   Iteration level files

      linkstats.csv.gz,plans.csv.gz,plans.xml.gz,rideHailFleet.csv.gz

- `beam.warmStart.skimsFilePaths`: This param is used for configuring type of skim and path of skim file
