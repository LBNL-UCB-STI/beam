...
The following inputs are optional and may fascilitate a truer conversio :
...
3. Create a another sub-directory to scenario directory and name if "r5". 
...
6. Make the following edits to siouxfalls.conf:

   Do a global search/replace and search for "beamville" and replace with your scenario (e.g. "siouxfalls").

   
Look for matsim.conversion section and update as required:
scenarioDirectory = "D:/tmp/siouxfalls" -> destination folder, must be copied to test/input to run BEAM

do we need to also update beam.inputDirectory????

populationFile = "Siouxfalls_population.xml" -> matsim population file name
matsimNetworkFile = "conversion-input/Siouxfalls_network_PT.xml" -> matsim network file name
generateVehicles = true -> If true conversion will use population data from above to generate default vehicles for agents
vehiclesFile = "Siouxfalls_vehicles.xml" -> If generateVehicles is false, specify matsim vehicles file name
defaultHouseholdIncome -> this data will be used for default household incomes
osmFile = "south-dakota-latest.osm.pbf" -> Open Street map source data file that should be clipped to scenario network
shapeConfig
shapeFile -> file name shape file package, e.g: for shape file name tz46_d00, there should be following files: tz46_d00.shp, tz46_d00.dbf, tz46_d00.shx
tazIdFieldName = "TZ46_D00_I" -> Field name for TAZ id in shape file
Look for beam.spatial section:
localCRS = "epsg:26914" -> CRS used for distance calculations, network, population and shape file must be in this format
boundingBoxBuffer = 10000 -> meters to pad bounding box around network
Run conversion tool
...
Copy to clipboard osmosis command generated by conversion tool
Download and install osmosis tool from: https://wiki.openstreetmap.org/wiki/Osmosis
Run osmosis command from previous step, e.g:
...
Point BEAM runner config parameter to bean config file, siouxfall.conf. Run simulation with generated data


Use beam.conf not template

replace beamville with siouxfalls

replace physsim-xml with yoru network " /conversion-input/Siouxfalls_network_PT.xml"
replace population-xml with your plans " /conversion-input/Siouxfalls_population.xml"


Need to set CRS

Network file must be changed in conf, after we do conversion 
...
/Users/critter/Downloads/siouxfalls/physsim-network.xml



change beam.routing.r5.osmFile to "scenario"


need toll-prices.csv in scenaior dir, can be empty with header linkId,toll
