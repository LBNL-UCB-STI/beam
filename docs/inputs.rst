...
TAZs, Scaling, and Physsim Tuning::
   ...
   # Parking Manager name (DEFAULT | HIERARCHICAL)
   beam.agentsim.taz.parkingManager.name = "DEFAULT"
   beam.agentsim.taz.parkingManager.hierarchical.numberOfClusters = 8
...
* beam.agentsim.taz.parkingManager.name: the name of the parking manager. HIERARCHICAL parking manager splits the TAZes into a number of clusters. This allows the users to speed up the searching for parking stalls. But as a tradeoff, it has degraded quality. Usually, 8-16 clusters can provide satisfactory quality on big numbers of TAZes.
* beam.agentsim.taz.parkingManager.hierarchical.numberOfClusters: the number of clusters for HIERARCHICAL parking manager.
